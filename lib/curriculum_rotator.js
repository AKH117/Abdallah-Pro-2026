...
/**
 * 🩺 11. Medical Curriculum Academic Lesson (شرح المنهج الجامعي - موديول القلب والأطفال Block 7)
 * متزامن ومطابق بنسبة 100% مع جدول الكلية لجروب 7 والتقدم الأكاديمي اليومي
 */
export async function getNextMedicalCurriculumLesson(userId = ADMIN_CHAT_ID) {
  // 1. Determine day of the week in Cairo
  const dayFormatter = new Intl.DateTimeFormat('ar-EG', { timeZone: 'Africa/Cairo', weekday: 'long' });
  const cleanDay = dayFormatter.format(new Date()).replace('الـ', '').replace('ال', '').replace(/[أإآ]/g, 'ا').trim();

  // 2. Day-to-curriculum mapping for Block 7 Group 7:
  // - الأحد / الإثنين: رسم القلب ومقدمة الكارديولوجي (CAD Intro & Normal ECG Blueprint)
  // - الثلاثاء: أمراض الصمام الميترالي (Mitral Valve MS & MR) وتقييم نمو الأطفال (Growth Assessment)
  // - الأربعاء: أمراض الصمام الأورطي (Aortic Valve AS & AR) واضطرابات نمو الأطفال
  // - الخميس / الجمعة / السبت: مراجعة شاملة وتثبيت متباعد لكافة موضوعات الأسبوع
  let dayKeywords = [];
  if (cleanDay.includes('احد') || cleanDay.includes('اثنين')) {
    dayKeywords = ['ecg', 'normal_ecg'];
  } else if (cleanDay.includes('ثلاثاء')) {
    dayKeywords = ['mitral', 'growth'];
  } else if (cleanDay.includes('اربعاء')) {
    dayKeywords = ['aortic', 'growth'];
  }

  if (dayKeywords.length > 0) {
    const matchedItems = MEDICAL_CURRICULUM.filter(item => {
      const targetStr = `${item.id || ''} ${item.topic || ''} ${item.title || ''}`.toLowerCase();
      return dayKeywords.some(kw => targetStr.includes(kw));
    });

    if (matchedItems.length > 0) {
      const chosenDayItem = await getNextRotatingCurriculumItem(`med_curriculum_${dayKeywords[0]}`, matchedItems, userId);
      if (chosenDayItem) return chosenDayItem;
    }
  }

  // 3. Fallback / Review cycle across all curriculum items
  const chosen = await getNextRotatingCurriculumItem('med_curriculum', MEDICAL_CURRICULUM, userId);
  return chosen;
}
