...
export function registerHandlers(bot) {
  ...
  bot.action(/^master_eng_(.+)$/, async (ctx) => {
    ...
    return ctx.editMessageText('👑 <b>عاش يا دكتور! تم نقل الكارت إلى (المتقن نهائياً 100%) ولن يتكرر إرساله مجدداً! 🎯</b>', { parse_mode: 'HTML' }).catch(() => {});
  });
  ...
  bot.action(/^fail_eng_(.+)$/, async (ctx) => {
    ...
    return ctx.editMessageText('🔄 <b>تمت إعادة جدولة الكلمة بعد 24 ساعة لتثبيتها في الذاكرة! 🧠</b>', { parse_mode: 'HTML' }).catch(() => {});
  ...
  bot.command(['english', 'eng', 'انجليزي', 'إنجليزي', 'vocab'], async (ctx) => {
    const { data: cards } = await supabase.from('english_spaced_flashcards').select('*').order('created_at', { ascending: false }).limit(15);
  ...
  bot.action('menu_eng_spaced', async (ctx) => {
    ...
    const { data: cards } = await supabase.from('english_spaced_flashcards').select('*').order('created_at', { ascending: false }).limit(25);
    ...
    else cards.forEach((c, idx) => msg += `${idx + 1}. 🌟 <b>${c.term_or_sentence}</b>\n   ➔ ${c.egyptian_translation} <i>[مستوى: ${c.repetition_level || 0}/6]</i>\n\n`);
