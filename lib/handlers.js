...
export function registerHandlers(bot) {
  ...
  bot.action(/^pass_eng_(.+?)_(\d+)$/, async (ctx) => {
    ...
    return ctx.editMessageText(`🎉 <b>إتقان ممتاز يا دكتور! تم رفع المستوى إلى (${nextLevel}/6) 🎯</b>`, {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
          [{ text: '➡️ الكارت التالي ⏭️', callback_data: 'review_eng_next' }],
  ...
  bot.action(/^fail_eng_(.+)$/, async (ctx) => {
    ...
    return ctx.editMessageText('🔄 <b>تمت إعادة جدولة الكلمة بعد 24 ساعة لتثبيتها في الذاكرة! 🧠</b>', {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
          [{ text: '➡️ الكارت التالي ⏭️', callback_data: 'review_eng_next' }],
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        [
          ...
          { text: '📚 قائمة الكروت', callback_data: 'menu_eng_spaced' }
        ]
      ]
    };
