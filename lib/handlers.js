...
export function registerHandlers(bot) {
  ...
  bot.on('text', async (ctx) => {
    ...
    function isMedicalErrorExplanation(raw) {
      if (!raw || typeof raw !== 'string' || raw.length < 100) return false;
      const t = raw.toLowerCase();
      let hits = 0;
      if (t.includes('ترجمة رأس السؤال') || t.includes('المصطلحات الصعبة') || t.includes('المصطلحات اللي ممكن تعمل أزمة') || t.includes('ترجمة السؤال')) hits++;
      if (t.includes('ليه الإجابة الصح') || t.includes('الإجابة الصح') || t.includes('السبب الفسيولوجي والأشعاعي') || t.includes('السبب الفسيولوجي')) hits++;
      if (t.includes('ليه باقي الإجابات') || t.includes('الإجابات التانية غلط') || t.includes('خيارات غلط') || t.includes('باقي الإجابات')) hits++;
      if (t.includes('تذكير بالمنهج') || t.includes('المنطقة المحيطة بالسؤال') || t.includes('ملخص علامات')) hits++;
      if (t.includes('نصائح ذهبية') || t.includes('تحشيشة') || t.includes('نيمونيك') || t.includes('التحشيشة الماسية')) hits++;
      return hits >= 2;
    }

    if (isMedicalErrorExplanation(text)) {
      const waitMsg = await ctx.reply('⏳ <i>جاري استيعاب وتفكيك شرح السؤال الطبي الخماسي وحفظه في سجل الأخطاء وبنك التحشيشات...</i>', { parse_mode: 'HTML' });
      try {
        const aiKeys = await getStoredAiKeys();
        ...
        if (Array.isArray(parsed.distractors)) {
          parsed.distractors.forEach(d => {
            distractorLines += `• ❌ <s>${d.option}</s>: ${d.reason}\n`;
          });
        }

        let respMsg = `🩺 <b>تم استيعاب وتفكيك السؤال الطبي وتحويله لكنز معرفي! 🎯✨</b>\n`;
        respMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        respMsg += `🏷️ <b>الموديول:</b> <code>[${parsed.course_code || 'CAD402'}]</code> • <b>${parsed.topic}</b>\n\n`;
        if (parsed.question_en) {
          respMsg += `❓ <b>السؤال:</b>\n<i>"${parsed.question_en}"</i>\n\n`;
        }
        if (parsed.question_ar) {
          respMsg += `🇸🇦 <b>الترجمة:</b> <i>"${parsed.question_ar}"</i>\n\n`;
        }
        respMsg += `✅ <b>الإجابة الصحيحة والميكانيزم:</b>\n<b>${parsed.correct_answer}</b>\n${parsed.correct_mechanism ? `<i>${parsed.correct_mechanism}</i>\n` : ''}\n`;
        if (distractorLines) {
          respMsg += `🔍 <b>تفكيك المشتتات والأفخاخ (${parsed.distractors?.length || 0}):</b>\n${distractorLines}\n`;
        }
        if (parsed.mnemonic_text) {
          respMsg += `💡 <b>التحشيشة المحفوظة في بنك النيمونيك:</b>\n« <b>${parsed.mnemonic_text}</b> »\n\n`;
        }
        if (parsed.golden_tip) {
          respMsg += `🌟 <b>النصيحة الذهبية:</b> ${parsed.golden_tip}\n\n`;
        }
        respMsg += `⏰ <i>تمت جدولة السؤال تلقائياً في التكرار المتباعد لتختبر فيه غداً لضمان إتقانه! 🚀</i>`;

        const vaultId = vaultRes?.id || 'last';
        const kb = {
          inline_keyboard: [
            [{ text: '⚡ اختبرني في المشتتات الآن (Distractor Quiz)', callback_data: `quiz_distractors_${vaultId}` }],
            [{ text: '💎 فتح بنك التحشيشات والـ Pearls', callback_data: 'menu_mnemonics' }],
            [{ text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }]
          ]
        };

        return ctx.reply(respMsg, { parse_mode: 'HTML', reply_markup: kb });
      } catch (err) {
        await ctx.deleteMessage(waitMsg.message_id).catch(() => {});
