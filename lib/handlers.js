...
return ctx.editMessageText('🔄 <b>تمت إعادة جدولة السؤال بعد 12 ساعة لتثبيته! 🩺</b>', { parse_mode: 'HTML' }).catch(() => {});
  ...
  bot.action('ack_read_calm', async (ctx) => {
    const fromId = ctx.from?.id;
    if (fromId) {
      const { addDoctorXp } = await import('./supabase.js');
      await addDoctorXp(fromId, 20, 'daily_pearl_read').catch(() => {});
    }
    await ctx.answerCbQuery('✅ بارك الله فيك ونفعك بما علمت! تم توثيق الاستيعاب والتدبر بنجاح (+20 XP) ✨').catch(() => {});
    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم الاستيعاب والتدبر بنجاح (+20 XP) 🌟', callback_data: 'noop_read' }]
      ]
    }).catch(() => {});
  });
