...
return ctx.editMessageText('🔄 <b>تمت إعادة جدولة السؤال بعد 12 ساعة لتثبيته! 🩺</b>', { parse_mode: 'HTML' }).catch(() => {});
  ...
  bot.action(/^ack_read_sharia_(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery('✅ تم تأكيد القراءة والاستيعاب بنجاح! (+20 XP)\n⏳ تمت جدولة سؤال الكويز التفاعلي ليختبرك البوت فيه بعد ساعتين (+2h) 🎯', { show_alert: true }).catch(() => {});

    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم استيعاب الدرس — مجدول للمراجعة بعد ساعتين (+2h) ⏳', callback_data: 'noop_read' }],
