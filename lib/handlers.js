...
export function registerHandlers(bot) {
  ...
  bot.on('poll_answer', async (ctx) => {
    try {
      ...
      let msg = '';
      if (result.isCorrect) {
        msg += `🎉 <b>إجابة ممتازة وصحيحة يا دكتور! 🩺✨ (+30 Doctor XP)</b>\n`;
      } else {
        msg += `⚠️ <b>إجابة غير دقيقة يا دكتور.. لا بأس! 🩺</b>\n`;
        msg += `📌 <i>تمت إعادة جدولة السؤال للمراجعة العاجلة غداً لتثبيت المفهوم في الذاكرة طويلة المدى.</i>\n`;
      }
      msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      if (cleanTopic) {
        msg += `📚 <b>الموضوع:</b> <b>${cleanTopic}</b>\n`;
      }
      msg += `🏆 <b>المرحلة:</b> [${result.nextLevel}/6] ${isMastered ? '👑 (تم الإتقان 100% مدى الحياة!)' : ''}\n`;
      ...
      if (fullExplanation) {
        msg += `📋 <b>الشرح الإكلينيكي والتشريحي الكامل (The 5-Step Anatomy):</b>\n\n`;
        msg += `${fullExplanation}\n\n`;
      }
      ...
      await bot.telegram.sendMessage(fromId, msg, { parse_mode: 'HTML', reply_markup: keyboard }).catch(async (err) => {
        console.warn('[poll_answer HTML Send Error]:', err.message);
        const plainMsg = msg.replace(/<[^>]*>/g, '');
        await bot.telegram.sendMessage(fromId, plainMsg, { reply_markup: keyboard }).catch(() => {});
      });
    } catch (e) {
