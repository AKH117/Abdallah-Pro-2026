...
export function registerHandlers(bot) {
            ...
            for (const item of q.difficult_terms) {
              ...
              if (term && term.trim()) {
                try {
                  ...
                  savedFlashcardTerms.push({ term: term.trim(), meaning: meaning.trim() });
                  termIdx++;
                } catch (fcErr) {
  ...
  bot.action(/^master_eng_(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery('👑 تم تعليمه كمتقن نهائياً!');
    return ctx.editMessageText('👑 <b>عاش يا دكتور! تم نقل الكارت إلى (المتقن نهائياً 100%) ولن يتكرر إرساله مجدداً! 🎯</b>', {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
  ...
  bot.action(/^pass_eng_(.+?)_(\d+)$/, async (ctx) => {
    ...
    let msg = `🎉 <b>إتقان ممتاز يا دكتور! 🇬🇧✨ (+20 English XP)</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
  ...
  bot.action(/^fail_eng_(.+)$/, async (ctx) => {
    ...
    let msg = `🔄 <b>تمت إعادة ضبط الكلمة لتثبيتها يا دكتور! 🧠</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
