import {
  getNextSahabiSpotlight,
  getNextScientificDiscipline,
  getNextAyahWithAsbab,
  getNextBukhariHadith,
  getNextPropheticSituation,
  getNextShariaCapsule,
  getNextPastRulerMistake,
  getNextStatesmanPearl,
  getNextPuritySpiritualFuel
} from './curriculum_rotator.js';
import { 
...
import {
  ...
  saveMedicalMnemonic,
  getMedicalMnemonics,
  saveMedicalErrorVault,
  updateUserAcademicGroup
} from './supabase.js';

import {
  ALL_COLLEGE_GROUPS_SCHEDULE,
  parseAcademicGroup,
  getScheduleForGroup
} from './college_schedules.js';
...
import {
  ...
  parseMedicalErrorExplanation,
  parseMedicalMnemonic
} from './ai_engine.js';
...
function getNextSpacedReviewDate(currentLevel, isSuccess) {
  ...
  if (!isSuccess) {
    return {
      nextLevel: 0,
      nextReviewDate: new Date(now + 24 * 3600 * 1000).toISOString(),
      isMastered: false
    };
  }

  const nextLevel = Math.min(6, (currentLevel || 0) + 1);
  let intervalHours = 120; // Level 1: 5 days

  switch (nextLevel) {
    case 1: intervalHours = 120; break;  // 5 days
    ...
    case 6:
      return {
        nextLevel: 6,
        nextReviewDate: new Date(now + 180 * 24 * 3600 * 1000).toISOString(),
        isMastered: true
      };
  }
...
export function registerHandlers(bot) {
  ...
  const _configuredMenuButtons = new Set();

  // 🛡️ Middleware: Multi-Tenant Profile & Subscription Gatekeeper
  bot.use(async (ctx, next) => {
    // 🎯 If update is poll_answer or has no chat, bypass interactive messaging gatekeepers immediately
    if (ctx.pollAnswer || ctx.updateType === 'poll_answer' || !ctx.chat) {
      return next();
    }
    ...
    // Ensure native persistent Web App Menu Button is set for this chat (only once per session)
    if (ctx.chat && ctx.chat.type === 'private' && !_configuredMenuButtons.has(fromId)) {
      _configuredMenuButtons.add(fromId);
    ...
    if (!profile.is_active) {
      ...
      if (ctx.callbackQuery) {
        await ctx.answerCbQuery('🔒 عفواً، انتهت صلاحية حسابك! يرجى تجديد الاشتراك للاستمرار في استخدام المنظومة.', { show_alert: true }).catch(() => {});
      }
      ...
      await bot.telegram.sendMessage(fromId, paywallMsg, {
        parse_mode: 'HTML',
  ...
  const defaultWebAppBtn = {
    type: 'web_app',
    text: '📱 لوحة التحكم',
    web_app: { url: 'https://akh117.github.io/Abdallah-Pro-2026/' }
  };
  bot.telegram.setChatMenuButton({ menuButton: defaultWebAppBtn }).catch(() => {});

  if (ADMIN_CHAT_ID) {
    bot.telegram.setChatMenuButton({
      chatId: Number(ADMIN_CHAT_ID),
      menuButton: defaultWebAppBtn
    }).catch(() => {});
  }
  ...
  function buildDynamicMenuKeyboard(profile, fromId) {
    ...
    const isSuperAdmin = isAdminUser(fromId);
    const userGroup = isSuperAdmin ? 'جروب 7' : (profile?.academic_group || null);
    const scheduleBtnText = userGroup ? `📅 قسم الجدول بتاعي (${userGroup})` : '📅 جدول السكاشن (اختر جروبك)';

    // Row 0: 📅 قسم الجدول بتاعي + 🧭 أعمل إيه دلوقتي؟
    rows.push([
      { text: scheduleBtnText, callback_data: 'btn_my_schedule' },
      { text: '🧭 أعمل إيه دلوقتي؟', callback_data: 'btn_what_to_do_now' }
    ]);

    // Row 1: Core Medical & Mnemonics
    const row1 = [
      { text: '🩺 كويزات ومذاكرة الطب', callback_data: 'menu_med_spaced' },
      { text: '💎 التحشيشات والـ Pearls', callback_data: 'menu_mnemonics' }
    ];
    rows.push(row1);
    if (prefs.english !== false) {
      rows.push([{ text: '🗣️ فلاش كاردز الإنجليزية', callback_data: 'menu_eng_spaced' }]);
    }

    // Row 2: Academic Schedule & Quran (if islamic enabled)
    const row2 = [];
    if (prefs.schedule !== false) {
      const row2Text = userGroup ? `📅 جدول مواعيدي وسكاشني (${userGroup})` : '📅 جدول السكاشن والراوندات';
      row2.push({ text: row2Text, callback_data: 'btn_my_schedule' });
    }
  ...
  bot.command(['rename', 'name', 'اسم'], async (ctx) => {
    ...
    return ctx.reply(successMsg, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🚀 فتح القائمة الرئيسية', callback_data: 'menu_main' }],
          [{ text: '📱 لوحة التحكم', web_app: { url: 'https://akh117.github.io/Abdallah-Pro-2026/' } }]
        ]
      }
    });
  });

  // 💎 Helper: Interactive Medical Mnemonics & Pearls Deck Viewer
  async function sendMnemonicsViewer(ctx, fromId, courseFilter = 'ALL', pageIndex = 0, isEdit = false) {
    const numId = Number(fromId) || ctx.from?.id;
    const cleanFilter = courseFilter ? courseFilter.toUpperCase().trim() : 'ALL';
    const list = await getMedicalMnemonics(numId, cleanFilter);

    if (!list || list.length === 0) {
      let emptyMsg = `💎 <b>بنك التحشيشات والـ Medical Pearls 🧠✨</b>\n`;
      emptyMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      emptyMsg += `لا توجد تحشيشات مسجلة حالياً في قسم <code>[${cleanFilter}]</code>.\n\n`;
      emptyMsg += `🎙️ <b>كيف تسجل تحشيشة جديدة؟</b>\n`;
      emptyMsg += `• سجل فويس وقول: <i>«سجل تحشيشة كذا كذا...»</i> أو ابدأها بتحشيشة على طول.\n`;
      emptyMsg += `• أو أرسل شرح سؤال غلط (النموذج الخماسي) وسيتم استخراج التحشيشة وحفظها تلقائياً!\n\n`;
      emptyMsg += `👇 <i>اختر قسماً آخر أو ابدأ بالتسجيل الآن:</i>`;

      const emptyKb = {
        inline_keyboard: [
          [
            { text: cleanFilter === 'ALL' ? '✅ الكل' : 'الكل', callback_data: 'mnem_filter_ALL_0' },
            { text: cleanFilter === 'CAD402' ? '✅ CAD402' : 'CAD402 🫀', callback_data: 'mnem_filter_CAD402_0' },
            { text: cleanFilter === 'PED401' ? '✅ PED401' : 'PED401 👶', callback_data: 'mnem_filter_PED401_0' }
          ],
          [
            { text: '🎙️ سجل تحشيشة بصوتك الآن', callback_data: 'record_mnemonic_prompt' },
            { text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }
          ]
        ]
      };
      ...
      if (isEdit && ctx.callbackQuery) {
        return ctx.editMessageText(emptyMsg, { parse_mode: 'HTML', reply_markup: emptyKb }).catch(() => {});
      }
      return ctx.reply(emptyMsg, { parse_mode: 'HTML', reply_markup: emptyKb });
    }

    const safeIndex = Math.max(0, Math.min(pageIndex, list.length - 1));
    const item = list[safeIndex];
    const cardNum = safeIndex + 1;
    const total = list.length;

    let msg = `💎 <b>بنك التحشيشات والـ Medical Pearls 🧠✨</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `🏷️ <b>الموديول:</b> <code>[${item.course_code || 'CAD402'}]</code> • <b>${item.topic}</b>\n\n`;
    msg += `💡 <b>التحشيشة الماسية (Mnemonic Hook):</b>\n`;
    msg += `« <b>${item.mnemonic_text}</b> »\n\n`;

    if (item.explanation_pearl) {
      msg += `🩺 <b>السريرية والـ Pearl:</b>\n`;
      msg += `${item.explanation_pearl}\n\n`;
    }

    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `📍 <b>بطاقة (${cardNum} من ${total})</b> ${item.type === 'user_saved' ? '⭐ <i>(من تسجيلاتك الخاصة)</i>' : '📚 <i>(من إسكيمات المنهج)</i>'}`;

    const prevIdx = safeIndex > 0 ? safeIndex - 1 : total - 1;
    const nextIdx = safeIndex < total - 1 ? safeIndex + 1 : 0;

    const keyboard = {
      inline_keyboard: [
        [
          { text: '⬅️ السابق', callback_data: `mnem_nav_${prevIdx}_${cleanFilter}` },
          { text: '🎲 عشوائي', callback_data: `mnem_random_${cleanFilter}` },
          { text: 'التالي ➡️', callback_data: `mnem_nav_${nextIdx}_${cleanFilter}` }
        ],
        [
          { text: cleanFilter === 'ALL' ? '✅ الكل' : 'الكل', callback_data: 'mnem_filter_ALL_0' },
          { text: cleanFilter === 'CAD402' ? '✅ CAD402' : 'CAD402 🫀', callback_data: 'mnem_filter_CAD402_0' },
        ...
        [
          { text: cleanFilter === 'RSD403' ? '✅ RSD403' : 'RSD403 🫁', callback_data: 'mnem_filter_RSD403_0' },
          { text: cleanFilter === 'HVD404' ? '✅ HVD404' : 'HVD404 🩸', callback_data: 'mnem_filter_HVD404_0' },
          { text: cleanFilter === 'SKL 7' ? '✅ طوارئ' : 'طوارئ SKL 🔬', callback_data: 'mnem_filter_SKL 7_0' }
        ],
        [
          { text: '🎙️ سجل تحشيشة جديدة بصوتك', callback_data: 'record_mnemonic_prompt' },
          { text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }
        ]
      ]
    };
  ...
  // 💎 Commands: /mnemonics, /pearls, /تحشيشات, /نيمونيك
  bot.command(['mnemonics', 'pearls', 'تحشيشات', 'نيمونيك'], async (ctx) => {
    const fromId = ctx.from?.id;
    return sendMnemonicsViewer(ctx, fromId, 'ALL', 0, false);
  });
  ...
  async function sendStudentStats(ctx, fromId) {
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '📱 فتح لوحة التحكم الكاملة', web_app: { url: 'https://akh117.github.io/Abdallah-Pro-2026/' } }],
        [
  ...
  // ==============================================================================
  // 📅 قسم الجدول بتاعي • د. عبدالله (جروب 7) - My Smart Schedule & Timeline Hub
  // ==============================================================================

  const G7_WEEKLY_MAP = {
    'أحد': {
      isCollegeDay: true,
      m1: { time: '08:45 - 10:25 ص', name: '🫀 محاضرة قلب (CAD Intro to Cardiology)', loc: 'مبنى 3 — مدرج 3315' },
      m2: { time: '10:30 - 12:10 م', name: '🫀 محاضرة رسم قلب (CAD Normal ECG)', loc: 'مبنى 3 — مدرج 3315' },
      s1: { time: '12:30 - 02:10 م', name: '💬 SGD CAD (مناقشة حالات)', loc: 'المستشفى — قاعة 14' },
      s2: { time: '02:15 - 03:55 م', name: '🩺 SGT CAD (تدريس إكلينيكي وجولة CCU)', loc: 'الأقسام والعيادات وعناية CCU' },
    ...
    'إثنين': {
      ...
      studyTarget: 'مذاكرة حرة لموديول القلب وتفريغ ومذاكرة محاضرات الأحد (CAD & Normal ECG)'
    },
    'ثلاثاء': {
      isCollegeDay: true,
      m1: { time: '08:45 - 10:25 ص', name: '🫀 محاضرة صمامات (CAD Mitral Valve Diseases)', loc: 'مبنى 3 — مدرج 3315' },
      m2: { time: '10:30 - 12:10 م', name: '👶 محاضرة نمو أطفال (PED Growth & Assessment)', loc: 'مبنى 3 — مدرج 3315' },
      s1: { time: '12:30 - 02:10 م', name: '🏥 Alex CAD (سكشن الإسكندرية)', loc: 'المستشفى — قاعة 15' },
      s2: { time: '02:15 - 03:55 م', name: '🩺 Clinical CAD (كلينيكال باطنة وقلب)', loc: 'المستشفى — قاعة 10' },
      studyTarget: 'أمراض الصمام الميترالي (Mitral Stenosis & Regurgitation) وتقييم نمو الأطفال'
    },
    'أربعاء': {
      isCollegeDay: true,
      m1: { time: '08:45 - 10:25 ص', name: '👶 محاضرة نمو أطفال (PED Growth Disorders)', loc: 'مبنى 3 — مدرج 3315' },
      m2: { time: '10:30 - 12:10 م', name: '🫀 محاضرة صمامات أورطي (CAD Aortic Valve Diseases)', loc: 'مبنى 3 — مدرج 3315' },
      s1: { time: '12:30 - 02:10 م', name: '🔬 Skill Lab 7 (معمل مهارات مجموعة 7)', loc: 'المستشفى — قاعة 4' },
      s2: { time: '02:15 - 03:55 م', name: '📋 Cases 7&8 (مناقشة حالات سريرية)', loc: 'مبنى 3 — قاعة 3103' },
      studyTarget: 'أمراض الصمام الأورطي (Aortic Stenosis & Regurgitation) وفحص الصدر'
    },
    'خميس': {
      isCollegeDay: true,
      m1: { time: '08:45 - 10:25 ص', name: '🫀 صمامات وثلاثي (CAD Tricuspid & Prosthetic)', loc: 'مبنى 3 — مدرج 3315' },
      m2: { time: '10:30 - 12:10 م', name: '🫀 تصلب ودهون (CAD Atherosclerosis & Lipids)', loc: 'مبنى 3 — مدرج 3315' },
      s1: { time: '12:30 - 02:10 م', name: '👶 SGT & SGD PED (سكشن أطفال)', loc: 'مبنى 2 — قاعة 2018' },
      s2: { time: '02:15 - 03:55 م', name: '👶 Clinical PED (كلينيكال أطفال)', loc: 'المستشفى — قاعة 12' },
      studyTarget: 'تصلب الشرايين والدهون والصمامات الصناعية وعيادات الأطفال'
    },
    'جمعة': {
      isCollegeDay: false,
      offReason: 'إجازة نهاية الأسبوع الرسمية 🕌',
      studyTarget: 'صلاة الجمعة وسورة الكهف ومراجعة أسبوعية متباعدة (SRS)'
    },
    'سبت': {
      isCollegeDay: false,
      offReason: 'لا توجد دراسة بالكلية (يوم التأسيس والمذاكرة العميقة بالمنزل) 👑',
      studyTarget: 'سبت التأسيس والمذاكرة العميقة لشباتر الكارديو وحل بنوك الأسئلة والكويزات'
    }
  };

  // 🏛️ College Groups Schedule Helpers
  function getGroupSelectionKeyboard() {
    ...
    for (let r = 0; r < 4; r++) {
      ...
      for (let c = 1; c <= 4; c++) {
        const gNum = r * 4 + c;
        row.push({ text: `جروب ${gNum}`, callback_data: `btn_set_group_${gNum}` });
      }
      rows.push(row);
    }
  ...
  function buildFullWeekScheduleMessage(userGroup, studentName) {
    const sch = getScheduleForGroup(userGroup);
    let fullMsg = `📋 <b>جدول الأسبوع الرسمي لـ ${userGroup} (Block 7) 🏛️</b>\n`;
    if (studentName) fullMsg += `👤 <b>الطالب:</b> ${studentName}\n`;
    fullMsg += `━━━━━━━━━━━━━━━━━━━━━\n\n`;
    ...
    for (const day of days) {
      ...
      fullMsg += `🗓️ <b>${day}:</b>\n`;
      if (d.isCollegeDay) {
        fullMsg += `• 08:45 - 10:25 ص: ${d.m1.name} (<b>${d.m1.loc}</b>)\n`;
        fullMsg += `• 10:30 - 12:10 م: ${d.m2.name} (<b>${d.m2.loc}</b>)\n`;
        fullMsg += `• 12:30 - 02:10 م: <b>${d.s1.name}</b> (<b>${d.s1.loc}</b>)\n`;
        fullMsg += `• 02:15 - 03:55 م: <b>${d.s2.name}</b> (<b>${d.s2.loc}</b>)\n\n`;
      } else {
        fullMsg += `• 08:45 - 12:10 م: ☕ مراجعة ذاتية واستذكار حر لموديول القلب\n`;
        fullMsg += `• 12:30 - 02:10 م: 🧪 <b>Open Lab</b> معمل المهارات (المستشفى — <b>قاعة 1</b>)\n\n`;
      }
    }

    fullMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    fullMsg += `💡 <i>الجمعة والسبت إجازة رسمية ومراجعات وتثبيت أسبوعي!</i>`;
    return fullMsg;
  }

  async function handleMyScheduleHub(ctx, edit = false) {
    const fromId = ctx.from?.id;
    if (!fromId) return;

    const isSuperAdmin = isAdminUser(fromId);
    const userProf = ctx.userProfile || (await getUserProfile(fromId));
    const studentName = isSuperAdmin ? 'د. عبدالله' : (userProf?.full_name || 'دكتور زميل');
    const userGroup = isSuperAdmin ? 'جروب 7' : (parseAcademicGroup(userProf?.academic_group) || null);

    // If regular user has no academic group selected yet, show group picker
    if (!userGroup && !isSuperAdmin) {
      let promptMsg = `📅 <b>أهلاً بك يا ${studentName} في قسم جدول السكاشن والمواعيد! 🏛️🩺</b>\n`;
      promptMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      promptMsg += `لم يتم اختيار مجموعتك الدراسية في الكلية حتى الآن.\n\n`;
      promptMsg += `👇 <b>اختر رقم مجموعتك الدراسية (من جروب 1 إلى 16):</b>\n`;
      promptMsg += `لسحب مواعيد سكاشنك وقاعاتك ومحاضراتك تلقائياً من جدول الكلية المعتمد وتفعيل التنبيهات المخصصة لك! 🚀`;

      const kb = getGroupSelectionKeyboard();
      if (edit) {
        return ctx.editMessageText(promptMsg, { parse_mode: 'HTML', reply_markup: kb }).catch(() => {
          return ctx.reply(promptMsg, { parse_mode: 'HTML', reply_markup: kb });
        });
      }
      return ctx.reply(promptMsg, { parse_mode: 'HTML', reply_markup: kb });
    }

    const currentGroup = userGroup || 'جروب 7';
    const sch = getScheduleForGroup(currentGroup);
    ...
    let todayKey = null;
    for (const k of ['أحد', 'إثنين', 'ثلاثاء', 'أربعاء', 'خميس', 'جمعة', 'سبت']) {
      if (dayName.includes(k)) { todayKey = k; break; }
    }
    const todayData = todayKey ? sch[todayKey] : null;

    let pendingTasks = [];
    let overdueTasks = [];
    let completedTodayStudy = false;

    try {
      const { data: tasks } = await supabase
        .from('daily_tasks')
        .select('*')
        .or(`category.ilike.%usr:${fromId}%,category.not.ilike.%usr:%`)
        .order('created_at', { ascending: false })
        .limit(20);

      if (tasks && tasks.length > 0) {
        for (const t of tasks) {
          const isDone = t.status === 'منجز' || t.status === 'done';
          const isToday = t.date === dateStr;
          if (t.category?.includes('مذاكرة_اليوم') && isToday && isDone) {
            completedTodayStudy = true;
          }
          if (!isDone) {
            if (t.date < dateStr || (t.reminder_count && t.reminder_count > 0)) {
              overdueTasks.push(t);
            } else if (isToday) {
              pendingTasks.push(t);
            }
      ...
      console.warn('[handleMyScheduleHub Tasks Warn]:', e.message);
    }

    let rightNowStatus = '';
    let nextUpStatus = '';

    if (todayData && todayData.isCollegeDay) {
      if (currentMins < 525) {
        rightNowStatus = `🌅 <b>فترة الصباح الباكر:</b> الاستعداد والانطلاق للكلية وجلسة التحضير السريعة.`;
        nextUpStatus = `⏰ <b>08:45 ص:</b> ${todayData.m1.name} (${todayData.m1.loc})`;
      } else if (currentMins >= 525 && currentMins < 625) {
        rightNowStatus = `🏛️ <b>أنت الآن في المحاضرة الأولى:</b> ${todayData.m1.name}\n📍 <b>المكان:</b> ${todayData.m1.loc}`;
        nextUpStatus = `⏰ <b>10:30 ص:</b> ${todayData.m2.name} (${todayData.m2.loc})`;
      } else if (currentMins >= 625 && currentMins < 630) {
        rightNowStatus = `🚶‍♂️ <b>استراحة فاصلة بين المحاضرتين:</b> الانتقال للمدرج.`;
        nextUpStatus = `⏰ <b>10:30 ص:</b> ${todayData.m2.name} (${todayData.m2.loc})`;
      } else if (currentMins >= 630 && currentMins < 730) {
        rightNowStatus = `🏛️ <b>أنت الآن في المحاضرة الثانية:</b> ${todayData.m2.name}\n📍 <b>المكان:</b> ${todayData.m2.loc}`;
        nextUpStatus = `⏰ <b>12:10 م:</b> صلاة الظهر واستراحة الغداء 🕌`;
      } else if (currentMins >= 730 && currentMins < 750) {
        rightNowStatus = `🕌 <b>استراحة صلاة الظهر والغداء وشحن الطاقة!</b>`;
        nextUpStatus = `⏰ <b>12:30 م:</b> ${todayData.s1.name} (<b>${todayData.s1.loc}</b>)`;
      } else if (currentMins >= 750 && currentMins < 850) {
        rightNowStatus = `🩺 <b>أنت الآن في سكشن/راوند ${currentGroup} الأول:</b>\n• <b>${todayData.s1.name}</b>\n📍 <b>المكان:</b> <b>${todayData.s1.loc}</b>`;
        nextUpStatus = `⏰ <b>02:15 م:</b> ${todayData.s2.name} (<b>${todayData.s2.loc}</b>)`;
      } else if (currentMins >= 850 && currentMins < 855) {
        rightNowStatus = `🚶‍♂️ <b>فاصل 5 دقائق:</b> التوجه لقاعة السكشن الثاني.`;
        nextUpStatus = `⏰ <b>02:15 م:</b> ${todayData.s2.name} (<b>${todayData.s2.loc}</b>)`;
      } else if (currentMins >= 855 && currentMins < 955) {
        rightNowStatus = `🩺 <b>أنت الآن في سكشن/راوند ${currentGroup} الثاني:</b>\n• <b>${todayData.s2.name}</b>\n📍 <b>المكان:</b> <b>${todayData.s2.loc}</b>`;
        nextUpStatus = `⏰ <b>04:00 م:</b> انتهاء دوام الكلية والعودة للمنزل 🍲`;
      } else if (currentMins >= 955 && currentMins < 1050) {
        rightNowStatus = `🍲 <b>فترة العودة والراحة:</b> تناول وجبة الغداء وقيلولة خفيفة لاستعادة النشاط.`;
        nextUpStatus = `⏰ <b>05:30 م:</b> بدء جلسة التركيز العميق (مذاكرة محاضرات اليوم) 📚`;
      } else if (currentMins >= 1050 && currentMins < 1200) {
        rightNowStatus = `📚 <b>أنت الآن في جلسة التركيز العميق 1 (Deep Focus):</b>\n🎯 <b>المستهدف:</b> تلخيص ومذاكرة: ${todayData.studyTarget}`;
        nextUpStatus = `⏰ <b>08:00 م:</b> صلاة العشاء واستراحة فنجان قهوة ☕`;
      } else if (currentMins >= 1200 && currentMins < 1230) {
        rightNowStatus = `🕌 <b>صلاة العشاء واستراحة خفيفة</b>`;
        nextUpStatus = `⏰ <b>08:30 م:</b> جلسة الأسئلة والتطبيق العملي وفلاش كاردز الكارديو 🩺`;
      } else if (currentMins >= 1230 && currentMins < 1350) {
        rightNowStatus = `🩺 <b>أنت الآن في جلسة التطبيق وحل الأسئلة (Practice Block):</b>\nحل كويزات الكارديو ومراجعة فلاش كاردز المصطلحات الطبية.`;
        nextUpStatus = `⏰ <b>10:30 م:</b> التقييم اليومي والاستعداد للنوم الصحي 🌌`;
      } else {
        rightNowStatus = `🌌 <b>وقت السكينة والنوم العميق:</b> احرص على النوم لحفظ ما تعلمته وتثبيته بالذاكرة طويلة المدى.`;
        if (todayKey === 'أحد') {
          nextUpStatus = `⏰ <b>غداً الإثنين:</b> لا توجد دراسة أو سكاشن بالكلية (إجازة رسمية للأنشطة واستذكار حر لموديول القلب بالمنزل) ☕`;
        } else if (todayKey === 'ثلاثاء') {
          nextUpStatus = `⏰ <b>الأربعاء 08:45 ص:</b> محاضرات نمو الأطفال وصمامات الأورطي بمدرج 3315 🏛️`;
        } else if (todayKey === 'أربعاء') {
          nextUpStatus = `⏰ <b>الخميس 08:45 ص:</b> محاضرات الصمامات والتصلب والدهون بمدرج 3315 🏛️`;
        } else if (todayKey === 'خميس') {
          nextUpStatus = `⏰ <b>غداً الجمعة:</b> إجازة رسمية مباركة (صلاة الجمعة وسورة الكهف واستراحة) 🕌`;
        }
      }
    } else {
      if (todayKey === 'إثنين') {
        rightNowStatus = `☕ <b>اليوم الإثنين:</b> لا توجد دراسة أو سكاشن رسمية بالكلية اليوم (إجازة رسمية مخصصة للأنشطة الطلابية واستذكار حر لموديول القلب بالمنزل).`;
        nextUpStatus = `⏰ <b>الثلاثاء 08:45 ص:</b> بدء اليوم الدراسي بالكلية بمدرج 3315 (محاضرات الصمامات ونمو الأطفال) 🏛️`;
      } else if (todayKey === 'جمعة') {
        rightNowStatus = `🕌 <b>يوم الجمعة المبارك:</b> صلاة الجمعة وسورة الكهف وراحة أسرية، ومراجعة أسبوعية متباعدة (SRS).`;
        nextUpStatus = `⏰ <b>السبت:</b> يوم التأسيس والمذاكرة العميقة بالمنزل (لا توجد دراسة بالكلية) 👑`;
      } else {
        rightNowStatus = `👑 <b>سبت التأسيس والمذاكرة العميقة:</b> إنهاء أي متأخرات وتثبيت شباتر الكارديو وحل بنوك الأسئلة بالمنزل.`;
        nextUpStatus = `⏰ <b>غداً الأحد 08:45 ص:</b> بداية أسبوع الكلية الجديد بمدرج 3315 (محاضرات ومناقشات القلب) 🏛️`;
      }
    }

    let msg = `📅 <b>قسم الجدول بتاعي • ${studentName} (${currentGroup}) 🩺✨</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `⏰ <b>الوقت الحالي بالقاهرة:</b> ${timeFormatted} (${dayName} - ${dateStr})\n\n`;

    msg += `📍 <b>أنت فين والمفروض تعمل إيه دلوقتي؟ (Right Now):</b>\n`;
    msg += `${rightNowStatus}\n\n`;

    msg += `⏭️ <b>السكشن / الموعد القادم (Next Up):</b>\n`;
    msg += `${nextUpStatus}\n\n`;

    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `📖 <b>مقرر المذاكرة المستهدف اليوم (Today's Study Target):</b>\n`;
    if (todayData?.studyTarget) {
      msg += `📌 <b>الموضوع:</b> ${todayData.studyTarget}\n`;
      msg += `📊 <b>الحالة:</b> ${completedTodayStudy ? '🟢 تم إنجاز مذاكرة اليوم بنجاح عاش يا دكتور! (+50 XP)' : '⏳ لم تُسجل المذاكرة بعد (اضغط بالأسفل عند إنهائها)'}\n\n`;
    } else {
      msg += `📌 مراجعة متباعدة وتثبيت ما تم تحصيله خلال الأسبوع.\n\n`;
    }

    if (overdueTasks.length > 0) {
      msg += `⚠️ <b>مهام أو التزامات متأخرة تحتاج حسم (${overdueTasks.length}):</b>\n`;
      overdueTasks.slice(0, 3).forEach((ot) => {
        const cleanT = (ot.title || '').replace(/\[usr:\d+\]/g, '').trim();
        msg += `• ${cleanT} <i>(${ot.date || 'سابقاً'})</i>\n`;
      });
      msg += `\n`;
    }

    if (pendingTasks.length > 0) {
      msg += `📝 <b>مهام اليوم قيد التنفيذ (${pendingTasks.length}):</b>\n`;
      pendingTasks.slice(0, 3).forEach((pt) => {
        const cleanT = (pt.title || '').replace(/\[usr:\d+\]/g, '').trim();
        msg += `• ${cleanT}\n`;
      });
      msg += `\n`;
    }

    msg += `💡 <i>كل قاعة وسكشن مخصصين لـ ${currentGroup} مبرمجين بدقة وفق جدول الكلية الرسمي!</i>`;

    const gNum = currentGroup.replace(/\D+/g, '') || '7';
    const keyboard = {
      inline_keyboard: [
        [
          { text: completedTodayStudy ? '✨ تم تأكيد المذاكرة اليوم' : '✅ أنجزت مذاكرة اليوم (+50 XP)', callback_data: 'btn_ack_today_study_done' },
          { text: '⏳ تأجيل المذاكرة لبكرة', callback_data: 'btn_defer_today_study' }
        ],
        [
          { text: `📋 جدول الأسبوع الكامل لـ ${currentGroup}`, callback_data: `btn_view_full_week_${gNum}` },
          { text: '🔄 تحديث الجدول', callback_data: 'btn_my_schedule' }
        ],
        [
          { text: '🏛️ تغيير مجموعتي الدراسية', callback_data: 'btn_choose_academic_group' },
          { text: '🧭 أعمل إيه دلوقتي؟', callback_data: 'btn_what_to_do_now' }
        ],
        [
          { text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }
  ...
  // Bind Commands and Actions for My Schedule Hub
  bot.command(['my_schedule', 'schedule', 'timetable', 'جدولي', 'الجدول', 'قسم_الجدول', 'مواعيدي', 'جدول_اليوم'], async (ctx) => {
    return handleMyScheduleHub(ctx, false);
  });

  bot.command(['group', 'sec', 'section', 'جروب', 'سكشن', 'مجموعة'], async (ctx) => {
    const fromId = ctx.from?.id;
    if (!fromId) return;
    const text = (ctx.message?.text || '').trim();
    const parts = text.split(/\s+/);
    const isSuperAdmin = isAdminUser(fromId);
    const studentName = ctx.userProfile?.full_name || (isSuperAdmin ? 'د. عبدالله' : 'دكتور زميل');

    if (parts.length > 1) {
      const detectedGroup = parseAcademicGroup(parts.slice(1).join(' '));
      if (detectedGroup) {
        await updateUserAcademicGroup(fromId, detectedGroup);
        if (ctx.userProfile) ctx.userProfile.academic_group = detectedGroup;
        const groupNum = detectedGroup.replace(/\D/g, '');
        let confirmMsg = `🏛️ <b>تم تحديد مجموعتك الدراسية بنجاح: ${detectedGroup} يا ${studentName}! 🎯✨</b>\n`;
        confirmMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        confirmMsg += `تم ربط منظومتك بجدول كلية الطب المعتمد (Block 7) لـ <b>${detectedGroup}</b>.\n`;
        confirmMsg += `ستصلك تنبيهات قاعات السكاشن والمحاضرات الخاصة بمجموعتك تلقائياً! 🚀`;
        const kb = {
          inline_keyboard: [
            [
              { text: '📅 جدول اليوم وقاعاتي', callback_data: 'btn_my_schedule' },
              { text: '📆 جدول الأسبوع كاملاً', callback_data: `btn_view_full_week_${groupNum}` }
            ],
            [
              { text: '🏛️ تغيير المجموعة', callback_data: 'btn_choose_academic_group' },
              { text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }
            ]
          ]
        };
        return ctx.reply(confirmMsg, { parse_mode: 'HTML', reply_markup: kb });
      }
    }

    let promptMsg = `🏛️ <b>تحديد أو تغيير المجموعة الدراسية يا ${studentName}:</b>\n`;
    promptMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    promptMsg += `اختر رقم مجموعتك الدراسية في كلية الطب (Block 7) لسحب مواعيدك وقاعاتك ومحاضراتك تلقائياً:`;
    const kb = getGroupSelectionKeyboard();
    return ctx.reply(promptMsg, { parse_mode: 'HTML', reply_markup: kb });
  });

  bot.action('btn_my_schedule', async (ctx) => {
    await ctx.answerCbQuery('📅 جاري فتح قسم الجدول والسكاشن...').catch(() => {});
    return handleMyScheduleHub(ctx, false);
  });

  bot.action('btn_choose_academic_group', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const fromId = ctx.from?.id;
    const isSuperAdmin = isAdminUser(fromId);
    const studentName = ctx.userProfile?.full_name || (isSuperAdmin ? 'د. عبدالله' : 'دكتور زميل');
  ...
  bot.action(/^btn_set_group_(\d+)$/, async (ctx) => {
    const fromId = ctx.from?.id;
    if (!fromId) return;
    const gNum = ctx.match[1];
    const groupName = `جروب ${gNum}`;
    await updateUserAcademicGroup(fromId, groupName);
    if (ctx.userProfile) ctx.userProfile.academic_group = groupName;
    await ctx.answerCbQuery(`✅ تم اعتماد ${groupName} بنجاح! 🏛️`).catch(() => {});
    return handleMyScheduleHub(ctx, true);
  });

  bot.action(/^btn_view_full_week(?:_g?(\d+))?$/, async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const fromId = ctx.from?.id;
    const isSuperAdmin = isAdminUser(fromId);
    let targetGroup = 'جروب 7';
    if (ctx.match && ctx.match[1]) {
      targetGroup = `جروب ${ctx.match[1]}`;
    } else if (!isSuperAdmin) {
      const userProf = ctx.userProfile || (await getUserProfile(fromId));
      targetGroup = parseAcademicGroup(userProf?.academic_group) || 'جروب 7';
    }
    const studentName = isSuperAdmin ? 'د. عبدالله' : (ctx.userProfile?.full_name || 'دكتور زميل');
    const fullMsg = buildFullWeekScheduleMessage(targetGroup, studentName);
    const kb = {
      inline_keyboard: [
        [{ text: '🔙 العودة لقسم جدولي', callback_data: 'btn_my_schedule' }],
        [{ text: '🏛️ تغيير مجموعتي الدراسية', callback_data: 'btn_choose_academic_group' }, { text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }]
      ]
    };
    return ctx.editMessageText(fullMsg, { parse_mode: 'HTML', reply_markup: kb }).catch(() => {
      return ctx.reply(fullMsg, { parse_mode: 'HTML', reply_markup: kb });
    });
  });

  bot.action('btn_ack_today_study_done', async (ctx) => {
    const fromId = ctx.from?.id;
    await ctx.answerCbQuery('🎉 عاش يا دكتور! تم تسجيل إنجاز مذاكرة اليوم بنجاح (+50 XP)').catch(() => {});
    if (fromId) {
      const { dateStr } = getCairoNow();
      await addDoctorXp(fromId, 50, 'daily_study_done').catch(() => {});
      await supabase.from('daily_tasks').insert({
        title: `مذاكرة مقرر اليوم في الكارديولوجي`,
        category: `مذاكرة_اليوم [usr:${fromId}]`,
        date: dateStr,
        status: 'منجز',
        target_duration_mins: 120
      }).catch(() => {});
    }
    return handleMyScheduleHub(ctx, true);
  });

  bot.action('btn_defer_today_study', async (ctx) => {
    await ctx.answerCbQuery('⏳ تم تأجيل المهمة إلى الغد لترتيب الأولويات').catch(() => {});
    return handleMyScheduleHub(ctx, true);
  });
  ...
  bot.action(/^admin_approve_(\d+)_(\d+)$/, async (ctx) => {
    ...
    await activateUserSubscription(targetId, days, 'موافقة وتفعيل من الإدارة');

    const confirmationText = `✅ <b>تم تفعيل الاشتراك للطالب <code>${targetId}</code> لمدة ${days} يوم بنجاح! 🎉</b>`;
    try {
      await ctx.editMessageText(confirmationText, { parse_mode: 'HTML' });
    } catch (_) {
      try {
        await ctx.editMessageCaption(confirmationText, { parse_mode: 'HTML' });
      } catch (__) {}
    }

    await bot.telegram.sendMessage(targetId, `🎉 <b>تم تأكيد وتفعيل اشتراكك في المنظومة (${days} يوم) بنجاح يا دكتور! 🩺✨</b>\n\nاضغط /menu للبدء والاستمتاع بكافة مميزات المنظومة.`, { parse_mode: 'HTML' }).catch(() => {});
  ...
  bot.action(/^admin_reject_(\d+)$/, async (ctx) => {
    if (Number(ctx.from?.id) !== ADMIN_CHAT_ID) return ctx.answerCbQuery('غير مصرح لك');
    const targetId = ctx.match[1];
    const rejectText = `❌ <b>تم رفض طلب التفعيل للطالب <code>${targetId}</code>.</b>`;
    try {
  ...
  bot.on(['voice', 'audio'], async (ctx) => {
    ...
    try {
      ...
      // 💎 Check if user was waiting to record a mnemonic or caption indicates mnemonic
      const isMnemonicWait = session?.state === 'waiting_mnemonic_voice';
      const isMnemonicCaption = /(?:تحشيش|نيمونيك|إسكيم|pearl)/i.test(captionText);
      if (isMnemonicWait || isMnemonicCaption) {
        if (session?.state === 'waiting_mnemonic_voice') {
        ...
        const mnemResult = await parseMedicalMnemonic(audioBuffer, aiKeys, true, studentName);
        await saveMedicalMnemonic({
          telegram_id: fromId,
          course_code: mnemResult.course_code || 'CAD402',
          topic: mnemResult.topic || 'تحشيشة صوتية',
          mnemonic_text: mnemResult.mnemonic_text,
          explanation_pearl: mnemResult.explanation_pearl,
          source: 'voice_note'
        });
        await ctx.deleteMessage(waitMsg.message_id).catch(() => {});

        let msg = `💎 <b>تم تسجيل التحشيشة الماسية في بنك النيمونيك بنجاح! 🧠✨</b>\n`;
        msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        msg += `🏷️ <b>الموديول:</b> <code>[${mnemResult.course_code || 'CAD402'}]</code>\n`;
        msg += `📌 <b>الموضوع:</b> <b>${mnemResult.topic}</b>\n\n`;
        msg += `💡 <b>التحشيشة:</b>\n« <b>${mnemResult.mnemonic_text}</b> »\n\n`;
        if (mnemResult.explanation_pearl) {
          msg += `🩺 <b>السريرية والـ Pearl:</b>\n${mnemResult.explanation_pearl}\n\n`;
        }
        msg += `✨ <i>تم حفظها في بنك التحشيشات والـ Pearls ويمكنك استعراضها دائماً عبر أمر /mnemonics أو /pearls!</i>`;

        const kb = {
          inline_keyboard: [
            [{ text: '💎 فتح بنك التحشيشات والـ Pearls', callback_data: 'menu_mnemonics' }],
            [{ text: '🎙️ تسجيل تحشيشة أخرى', callback_data: 'record_mnemonic_prompt' }],
            [{ text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }]
          ]
        };
  ...
  // ⏰ Deterministic Fast Arabic Reminder Parser (0 Tokens, < 1ms)
  function parseArabicReminderInstant(text) {
    if (!text || typeof text !== 'string') return null;
    const raw = text.trim();
    const lower = raw.replace(/[،,]/g, ' ');

    const triggerRegex = /(?:^|\s)(?:فكرني|فكرنى|ذكرني|ذكرنى|نبهني|نبهنى|ابقى\s+فكرني|ابقى\s+ذكرني|عايزك\s+تفكرني|عايزك\s+تذكرني|تذكير\s+بـ?|جدولي\s+(?:ميعاد|معاد)|عايزك\s+تجدولي\s+(?:ميعاد|معاد))\s+/i;
    if (!triggerRegex.test(lower)) return null;
    ...
    // 1. Relative delay: 'بعد X'
    const relativeRegex = /بعد\s+(نصف\s+ساعة|نص\s+ساعة|ربع\s+ساعة|ثلث\s+ساعة|دقيقة|دقيقتين|ساعة|ساعتين|(\d+)\s*(?:دقايق|دقائق|دقيقة)|(\d+)\s*(?:ساعات|ساعة))/i;
    const relMatch = lower.match(relativeRegex);

    if (relMatch) {
      const timePhrase = relMatch[1].trim();
      if (/^دقيقة$/.test(timePhrase)) delayMinutes = 1;
      else if (/^دقيقتين$/.test(timePhrase)) delayMinutes = 2;
      else if (/^ربع\s+ساعة$/.test(timePhrase)) delayMinutes = 15;
      else if (/^ثلث\s+ساعة$/.test(timePhrase)) delayMinutes = 20;
      else if (/^(?:نصف|نص)\s+ساعة$/.test(timePhrase)) delayMinutes = 30;
      else if (/^ساعة$/.test(timePhrase)) delayMinutes = 60;
      else if (/^ساعتين$/.test(timePhrase)) delayMinutes = 120;
      else if (relMatch[2]) delayMinutes = parseInt(relMatch[2], 10);
      else if (relMatch[3]) delayMinutes = parseInt(relMatch[3], 10) * 60;

      topic = raw
        .replace(triggerRegex, '')
        .replace(/بعد\s+(?:نصف\s+ساعة|نص\s+ساعة|ربع\s+ساعة|ثلث\s+ساعة|دقيقة|دقيقتين|ساعة|ساعتين|\d+\s*(?:دقايق|دقائق|دقيقة|ساعات|ساعة))/gi, '')
        .replace(/من\s+دلوقتي|من\s+الآن/gi, '')
        .replace(/^(?:إن|إني|ان|اني|بـ|ب|أني|اني|عشان|علشان)\s+/i, '')
        .replace(/[!؟\.\,\،]+/g, ' ')
        .trim();

      targetDateObj = new Date(nowMs + delayMinutes * 60 * 1000);
    }

    // 2. Prayer-relative: 'بعد (صلاة )?(الفجر|الظهر|العصر|المغرب|العشاء)'
    const prayerMatch = lower.match(/بعد\s+(?:صلاة\s+)?(الفجر|الظهر|الضهر|العصر|المغرب|العشاء|العشا)/i);
    if (!relMatch && prayerMatch) {
      const pName = prayerMatch[1].replace('الضهر', 'الظهر').replace('العشا', 'العشاء');
      const prayers = getCairoPrayerTimes();
      const pKey = { 'الفجر': 'fajr', 'الظهر': 'dhuhr', 'العصر': 'asr', 'المغرب': 'maghrib', 'العشاء': 'isha' }[pName];
      const pTime = prayers.times ? prayers.times[pKey] : null;
      if (pTime) {
        const [h, m] = pTime.split(':').map(Number);
        const cairoDateStr = now.toLocaleDateString('en-CA', { timeZone: 'Africa/Cairo' });
        targetDateObj = new Date(`${cairoDateStr}T${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:00+03:00`);
        targetDateObj = new Date(targetDateObj.getTime() + 15 * 60 * 1000); // 15 mins after azan
        if (targetDateObj.getTime() < nowMs) {
          targetDateObj = new Date(targetDateObj.getTime() + 24 * 3600 * 1000);
        }
        delayMinutes = Math.round((targetDateObj.getTime() - nowMs) / 60000);

        topic = raw
          .replace(triggerRegex, '')
          .replace(/بعد\s+(?:صلاة\s+)?(الفجر|الظهر|الضهر|العصر|المغرب|العشاء|العشا)/gi, '')
          .replace(/^(?:إن|إني|ان|اني|بـ|ب|أني|اني|عشان|علشان)\s+/i, '')
          .replace(/[!؟\.\,\،]+/g, ' ')
          .trim();
      }
    ...
    // 3. Clock time: 'الساعة X'
    const clockMatch = lower.match(/الساعة\s+(\d{1,2})(?::(\d{2}))?\s*(نص|ونص|ربع|وربع|تلت|وتلت)?\s*(الصبح|صباحا|صباحاً|مساء|مساءً|بليل|بالليل|عصرا|عصراً|الظهر|الضهر)?/i);
    if (!relMatch && !prayerMatch && clockMatch) {
      let hour = parseInt(clockMatch[1], 10);
      let min = clockMatch[2] ? parseInt(clockMatch[2], 10) : 0;
      const frac = clockMatch[3] || '';
      if (frac.includes('نص')) min = 30;
      else if (frac.includes('ربع')) min = 15;
      else if (frac.includes('تلت')) min = 20;

      const period = clockMatch[4] || '';
      if (period.includes('مساء') || period.includes('بليل') || period.includes('عصر')) {
        if (hour < 12) hour += 12;
      } else if (period.includes('الصبح') || period.includes('صباح')) {
        if (hour === 12) hour = 0;
      } else {
        const cairoHour = parseInt(now.toLocaleTimeString('en-GB', { timeZone: 'Africa/Cairo', hour: '2-digit', hour12: false }), 10);
        if (hour <= 12 && cairoHour >= 12 && (hour + 12) > cairoHour) {
          hour += 12;
        }
      }

      const cairoDateStr = now.toLocaleDateString('en-CA', { timeZone: 'Africa/Cairo' });
      targetDateObj = new Date(`${cairoDateStr}T${String(hour).padStart(2, '0')}:${String(min).padStart(2, '0')}:00+03:00`);
      if (targetDateObj.getTime() < nowMs) {
        targetDateObj = new Date(targetDateObj.getTime() + 24 * 3600 * 1000);
      }
      delayMinutes = Math.round((targetDateObj.getTime() - nowMs) / 60000);
      ...
      topic = raw
        .replace(triggerRegex, '')
        .replace(/الساعة\s+\d{1,2}(?::\d{2})?\s*(?:نص|ونص|ربع|وربع|تلت|وتلت)?\s*(?:الصبح|صباحا|صباحاً|مساء|مساءً|بليل|بالليل|عصرا|عصراً|الظهر|الضهر)?/gi, '')
        .replace(/^(?:إن|إني|ان|اني|بـ|ب|أني|اني|عشان|علشان)\s+/i, '')
        .replace(/[!؟\.\,\\,]+/g, ' ')
        .trim();
    }
    ...
    if (targetDateObj && !isNaN(targetDateObj.getTime())) {
      return {
        title: topic || 'تذكير موعد',
        delayMinutes: Math.max(1, delayMinutes || 1),
        dueDatetime: targetDateObj.toISOString(),
        cairoTime: targetDateObj.toLocaleTimeString('ar-EG', { timeZone: 'Africa/Cairo', hour: '2-digit', minute: '2-digit', hour12: true })
      };
    }
    return null;
  }
  ...
  // ==============================================================================
  // ⌨️ 3. Text Messages Handler & Telegram Split Chunk Debouncer
  // ==============================================================================

  // ⚡ In-memory Debounce buffer for multi-part Telegram messages (>4096 chars)
  const _userTextChunks = new Map();
  const CHUNK_DEBOUNCE_MS = 1400;

  function isMedicalErrorExplanation(raw) {
    if (!raw || typeof raw !== 'string') return false;
    const t = raw.toLowerCase();

    // 1. Explicit Question markers (e.g. السؤال السابع, السؤال العاشر, Question 7, Q1, 7., etc.)
    const hasQuestionMarker = /(?:❓|السؤال(?:\s+[\u0600-\u06FF\w]+)?|سؤال\s*\d*|\bquestion\s*\d*|\bcase vignette\b|^\s*(?:❓|\d+[\.\-\)]|\(\d+\))\s*(?:السؤال|question)?)/i.test(raw);

    // 2. Mnemonic / Pearl / Golden tip markers
    const hasMnemonicMarker = /(?:تحشيشة|نيمونيك|التحشيشة الماسية|نصائح ذهبية|\bgolden tip\b|\bpearl\b|إسكيمة|إسكيم)/i.test(raw);

    // 3. Structural indicators from the 5-step framework
    const hasStepEmojis = /[1-5]️⃣|[1-5]-|أولاً|ثانياً|ثالثاً|رابعاً|خامساً/i.test(raw);
    const hasCoreKeywords = /ترجمة رأس السؤال|المصطلحات|الإجابة الصح|السبب التشريحي|السبب الفسيولوجي|المشتتات|باقي الإجابات|تذكير بالمنهج|حدود رسمة القلب|خيارات غلط|صح ليه|غلط ليه/i.test(raw);

    // 🎯 Decisive match combinations:
    if (hasQuestionMarker && hasMnemonicMarker) return true;
    if (hasQuestionMarker && (hasStepEmojis || hasCoreKeywords)) return true;
    if (hasMnemonicMarker && (hasStepEmojis || hasCoreKeywords)) return true;
    if (hasStepEmojis && hasCoreKeywords) return true;

    // 4. Split chunk indicators (e.g. part 2 of a split Telegram message)
    if (/^(?:الحد الأيسر|الحد الأيمن|5️⃣|4️⃣|3️⃣|2️⃣|إجابة\s+.*غلط|التحشيشة الماسية|نصائح ذهبية)/i.test(raw)) return true;

    // 5. If starts with question text and is a long explanation (>200 chars) with medical terms
    if (hasQuestionMarker && raw.length > 200 && /(?:ECG|STEMI|leads|artery|ventricle|atrium|CXR|cardiac|patient|occluded|heart|aortic|pulmonary|valve|مريض|تشخيص|شريان|قلب|أشعة)/i.test(raw)) return true;

    // 6. Fallback hit counter
    let hits = 0;
    if (t.includes('ترجمة') || t.includes('المصطلحات')) hits++;
    if (t.includes('الإجابة الصح') || t.includes('السبب التشريحي') || t.includes('السبب الفسيولوجي') || t.includes('صح ليه')) hits++;
    if (t.includes('المشتتات') || t.includes('باقي الإجابات') || t.includes('خيارات غلط') || t.includes('غلط ليه') || (t.includes('إجابة') && t.includes('غلط'))) hits++;
    if (t.includes('تذكير بالمنهج') || t.includes('المنطقة المحيطة') || t.includes('رسم القلب') || t.includes('أشعة الصدر')) hits++;
    if (t.includes('نصائح ذهبية') || t.includes('تحشيشة') || t.includes('نيمونيك')) hits++;
  ...
  bot.on('text', async (ctx) => {
    ...
    // Detect if this message looks like a multi-chunk candidate (split messages in Telegram)
    const isChunkCandidate = isMedicalErrorExplanation(text) ||
      text.length > 1800 ||
      /^(?:الحد\s+|[1-5]️⃣|[1-5]-|إجابة\s+|تحشيشة\s+|نصائح\s+|التحشيشة\s+|4️⃣|5️⃣)/i.test(text);

    const activeBuffer = _userTextChunks.get(fromId);
    if (activeBuffer || isChunkCandidate) {
      if (activeBuffer) {
        clearTimeout(activeBuffer.timer);
        activeBuffer.text += '\n\n' + text;
      } else {
        _userTextChunks.set(fromId, {
          text,
          timer: null,
          ctx
        });
      ...
      const currentBuffer = _userTextChunks.get(fromId);
      currentBuffer.timer = setTimeout(async () => {
        const fullText = currentBuffer.text;
        _userTextChunks.delete(fromId);
        try {
          await processUserTextWithDebounce(currentBuffer.ctx, fullText, fromId);
        } catch (e) {
          console.error('[Debounced Text Processing Error]:', e);
        }
      }, CHUNK_DEBOUNCE_MS);
      return;
    }

    return processUserTextWithDebounce(ctx, text, fromId);
  });

  async function processUserTextWithDebounce(ctx, text, fromId) {
    const session = fromId ? await getUserSession(fromId) : null;
    const studentName = ctx.userProfile?.full_name || (Number(fromId) === ADMIN_CHAT_ID ? 'د. عبدالله' : 'دكتور زميل');

    // 🩺 0. Medical Question Error Breakdown & Explanation Handler (5-Part Schema)
    if (isMedicalErrorExplanation(text)) {
      const waitMsg = await ctx.reply('⏳ <i>جاري استيعاب وتفكيك شرح الأسئلة الطبية بالنموذج الخماسي وحفظها في خزانة الأخطاء وبنك التحشيشات...</i>', { parse_mode: 'HTML' });
      try {
        const aiKeys = await getStoredAiKeys();
        const parsed = await parseMedicalErrorExplanation(text, aiKeys, studentName);
        const questions = Array.isArray(parsed.questions) && parsed.questions.length > 0
          ? parsed.questions
          : (parsed.question_en ? [parsed] : []);

        if (questions.length === 0) {
          throw new Error('لم يتم العثور على أسئلة مفككة في الرسالة');
        }

        const savedItems = [];
        const savedFlashcardTerms = [];
        let termIdx = 0;

        for (const q of questions) {
          const vaultRes = await saveMedicalErrorVault({
            telegram_id: fromId,
            course_code: q.course_code || 'CAD402',
            topic: q.topic || 'سؤال سريري مفكك',
            question_en: q.question_en,
            question_ar: q.question_ar,
            difficult_terms: q.difficult_terms || [],
            correct_answer: q.correct_answer,
            correct_mechanism: q.correct_mechanism,
            distractors: q.distractors || [],
            curriculum_context: q.curriculum_context,
            golden_tip: q.golden_tip,
            mnemonic_text: q.mnemonic_text
          });

          // 📚 2. Save "المصطلحات اللي ممكن تعمل أزمة" into Spaced Flashcards
          if (Array.isArray(q.difficult_terms) && q.difficult_terms.length > 0) {
            for (const item of q.difficult_terms) {
              const term = typeof item === 'string' ? item : item.term;
              const meaning = typeof item === 'string' ? 'مصطلح سريري هام' : (item.meaning || item.translation || '');
              if (term && term.trim()) {
                try {
                  ...
                  const { data: exCard } = await supabase
                    ...
                    .ilike('term_or_sentence', cleanT)
                    .maybeSingle();

                  if (!exCard) {
                    const staggerMins = Math.min(24 * 60, 20 + termIdx * 30);
                    const nextReview = new Date(Date.now() + staggerMins * 60 * 1000).toISOString();
                    await supabase.from('english_spaced_flashcards').insert({
                      term_or_sentence: cleanT,
                      egyptian_translation: meaning.trim() || 'مصطلح سريري هام',
                      example_sentence: q.question_en ? q.question_en.slice(0, 300) : null,
                      usage_context: `[UID:${fromId}] [${q.course_code || 'CAD402'}] مصطلح طبي سريري من السؤال`,
                      repetition_level: 0,
                      next_review_at: nextReview
                    });
                    savedFlashcardTerms.push({ term: cleanT, meaning: meaning.trim() });
                    termIdx++;
                  }
                } catch (fcErr) {
                  console.warn('Difficult term flashcard insert warn:', fcErr.message);
                }
              }
            }
          }
          ...
          savedItems.push({
            ...q,
            vaultId: vaultRes?.id || 'last'
          });
        }

        await ctx.deleteMessage(waitMsg.message_id).catch(() => {});

        let respMsg = `🩺 <b>تم استيعاب وتفكيك (${savedItems.length}) أسئلة طبية بالنموذج الخماسي بنجاح! 🎯✨</b>\n`;
        respMsg += `━━━━━━━━━━━━━━━━━━━━━\n\n`;

        savedItems.forEach((q, idx) => {
          let distractorLines = '';
          if (Array.isArray(q.distractors)) {
            q.distractors.forEach(d => {
              const opt = typeof d === 'string' ? d : d.option;
              const reason = typeof d === 'string' ? '' : d.reason;
              distractorLines += `   • ❌ <s>${opt}</s>: ${reason}\n`;
            });
          }

          let termsLines = '';
          if (Array.isArray(q.difficult_terms) && q.difficult_terms.length > 0) {
            q.difficult_terms.forEach(t => {
              const term = typeof t === 'string' ? t : t.term;
              const meaning = typeof t === 'string' ? '' : t.meaning;
              termsLines += `   • 🏷️ <b>${term}</b>: ${meaning}\n`;
            });
          }

          respMsg += `📌 <b>السؤال (${idx + 1}):</b> <code>[${q.course_code || 'CAD402'}]</code> • <b>${q.topic}</b>\n`;
          if (q.question_en) respMsg += `❓ <i>"${q.question_en}"</i>\n`;
          if (q.question_ar) respMsg += `🇸🇦 <b>الترجمة:</b> <i>"${q.question_ar}"</i>\n`;
          if (termsLines) respMsg += `📚 <b>المصطلحات اللي ممكن تعمل أزمة (فلاش كاردز):</b>\n${termsLines}`;
          respMsg += `✅ <b>الإجابة الصحيحة:</b> <b>${q.correct_answer}</b>\n`;
          if (q.correct_mechanism) respMsg += `🔬 <b>الميكانيزم:</b> <i>${q.correct_mechanism}</i>\n`;
          if (distractorLines) respMsg += `🔍 <b>المشتتات الخاطئة:</b>\n${distractorLines}`;
          if (q.mnemonic_text) respMsg += `💡 <b>التحشيشة الماسية:</b>\n« <b>${q.mnemonic_text}</b> »\n`;
          if (q.golden_tip) respMsg += `🌟 <b>النصيحة الذهبية:</b> <i>${q.golden_tip}</i>\n`;
          if (idx < savedItems.length - 1) respMsg += `\n─────────────────────\n\n`;
        });

        respMsg += `\n⏰ <i>تمت جدولة جميع الأسئلة تلقائياً في التكرار المتباعد، وحفظ التحشيشات في بنكك والويب آب! 🚀</i>`;
        if (savedFlashcardTerms.length > 0) {
          respMsg += `\n🗣️ <i>تم حفظ (${savedFlashcardTerms.length}) مصطلحات سريرية في فلاش كاردز التكرار المتباعد لتثبيتها! 🧠</i>`;
        }

        const inline_keyboard = [];
        savedItems.forEach((item, idx) => {
          inline_keyboard.push([{ text: `⚡ اختبرني في مشتتات السؤال (${idx + 1})`, callback_data: `quiz_distractors_${item.vaultId}` }]);
        });
        inline_keyboard.push([
          { text: '💎 بنك التحشيشات والـ Pearls', callback_data: 'menu_mnemonics' },
          { text: '🗣️ فلاش كاردز المصطلحات', callback_data: 'menu_english_hub' }
        ]);
        inline_keyboard.push([{ text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }]);

        return ctx.reply(respMsg, { parse_mode: 'HTML', reply_markup: { inline_keyboard } });
      } catch (err) {
        await ctx.deleteMessage(waitMsg.message_id).catch(() => {});
        return ctx.reply(`⚠️ <b>تعذر تفكيك شرح السؤال:</b> ${err.message}`, { parse_mode: 'HTML' });
      }
    }


    // 🏛️ 0.05 Academic Group Direct Intent / Registration (Zero-Latency, 0 AI tokens)
    const cleanUserText = text.trim();
    const isGroupState = session?.state === 'waiting_academic_group';
    const isShortGroupText = cleanUserText.length <= 40;
    const isGroupMention = /(?:^|\s)(?:أنا|انا)?\s*(?:في\s+)?(?:جروب|سكشن|مجموعة|مجموعتي|group|sec|section|\bg)\s*([1-9]|1[0-6])(?![0-9])/i.test(cleanUserText) ||
                           /^(?:جروب|سكشن|مجموعة|group|sec)?\s*([1-9]|1[0-6])$/i.test(cleanUserText);

    if ((isGroupState || isGroupMention) && isShortGroupText) {
      const detectedGroup = parseAcademicGroup(cleanUserText);
      if (detectedGroup) {
        if (isGroupState) {
          await setUserSession(fromId, { state: 'idle' });
        }
        ...
        confirmMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        confirmMsg += `تم ربط منظومتك بجدول كلية الطب المعتمد (Block 7) لـ <b>${detectedGroup}</b>.\n`;
        confirmMsg += `ستصلك تنبيهات قاعات السكاشن والمحاضرات الخاصة بمجموعتك تلقائياً دون تداخل مع المجموعات الأخرى! 🚀`;
    ...
    // 💎 0.1 Mnemonic Text Dictation Intent
    const isMnemState = session?.state === 'waiting_mnemonic_text';
    const isMnemTextMatch = /(?:^|\s)(?:سجل\s+تحشيشة|نيمونيك\s+جديدة|تحشيشة\s+جديدة|سجل\s+نيمونيك|عايز\s+اسجل\s+تحشيشة|تحشيشة\s+لـ|إسكيمة\s+لـ)/i.test(text);
    if (isMnemState || isMnemTextMatch) {
      if (isMnemState) {
        await setUserSession(fromId, { state: 'idle' });
      }
      const waitMsg = await ctx.reply('⏳ <i>جاري تحليل التحشيشة واستخراج الـ Pearl وحفظها في بنك النيمونيك...</i>', { parse_mode: 'HTML' });
      try {
        const aiKeys = await getStoredAiKeys();
        const mnemResult = await parseMedicalMnemonic(text, aiKeys, false, studentName);
        await saveMedicalMnemonic({
          telegram_id: fromId,
          course_code: mnemResult.course_code || 'CAD402',
          topic: mnemResult.topic || 'تحشيشة سريرية',
          mnemonic_text: mnemResult.mnemonic_text,
          explanation_pearl: mnemResult.explanation_pearl,
          source: 'manual'
        });
        ...
        await ctx.deleteMessage(waitMsg.message_id).catch(() => {});
        return ctx.reply(`⚠️ <b>تعذر تسجيل التحشيشة:</b> ${err.message}`, { parse_mode: 'HTML' });
      }
    }
    ...
    if (!session?.state || session.state === 'idle') {
      ...
      // ⏰ Fast-Track Instant Reminder Guard (0 Tokens, instant response, guaranteed schedule)
      const instantReminder = parseArabicReminderInstant(text);
      if (instantReminder) {
        const todayDate = getCairoToday();
        ...
        const { data: row } = await supabase.from('appointments_and_reminders').insert({
          title: instantReminder.title,
          due_datetime: instantReminder.dueDatetime,
          remind_at: instantReminder.dueDatetime,
          notes: `[usr:${fromId}] تذكير مباشر مجدول`,
          date: todayDate,
          is_notified: false,
          is_completed: false
        ...
        if (row?.id) {
          ...
          undoHistory[undoActionId] = {
            ...
            timestamp: Date.now(),
            date: todayDate,
            items: [{ table: 'appointments_and_reminders', id: row.id, summary: `⏰ تذكير: ${instantReminder.title}` }],
            financeItems: [],
            financeReversions: [],
            summaryList: [`⏰ <b>تذكير مجدول:</b> <b>${instantReminder.title}</b> (${instantReminder.cairoTime})`]
          };
          currentSession.undo_history = undoHistory;
          await setUserSession(fromId, currentSession);

          // Fast-track in-memory timer for delays <= 120 minutes (2 hours)
          const delayMs = new Date(instantReminder.dueDatetime).getTime() - Date.now();
          if (delayMs > 0 && delayMs <= 120 * 60 * 1000) {
            setTimeout(async () => {
              try {
                ...
                if (checkRow && !checkRow.is_notified && !checkRow.is_completed) {
                  ...
                  let alertMsg = `⏰ <b>حان موعد التذكير يا ${studentName}! 🔔✨</b>\n`;
                  alertMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
                  alertMsg += `📌 <b>${cleanTitle}</b>\n\n`;
                  alertMsg += `👇 <i>اضغط لتأكيد الإنجاز أو تأجيل التذكير:</i>`;
                  const kb = {
                    inline_keyboard: [
                      [
                        { text: '✅ تم إنجاز الأمر', callback_data: `ack_appt_done_${checkRow.id}` },
                        { text: '⏳ تأجيل 15 دقيقة', callback_data: `snooze_appt_${checkRow.id}_15` }
                      ]
                    ]
                  };
                  await bot.telegram.sendMessage(fromId, alertMsg, { parse_mode: 'HTML', reply_markup: kb }).catch(() => {});
                }
              } catch (err) {
                console.warn('Instant reminder timer error:', err.message);
              }
            }, Math.max(1000, delayMs));
          }
        }

        const delayDesc = instantReminder.delayMinutes >= 60
          ? `${(instantReminder.delayMinutes / 60).toFixed(1).replace('.0', '')} ساعة`
          : `${instantReminder.delayMinutes} دقيقة`;

        let confirmMsg = `⏰ <b>تم تثبيت وجدولة التذكير بنجاح يا ${studentName}! 🔔✨</b>\n`;
        confirmMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        confirmMsg += `📌 <b>الموضوع:</b> <b>${instantReminder.title}</b>\n`;
        confirmMsg += `⏱️ <b>الموعد:</b> بعد ${delayDesc} في تمام الساعة <b>${instantReminder.cairoTime}</b>\n\n`;
        confirmMsg += `✨ <i>سأقوم بإرسال إشعار وتنبيه لك في هذا الموعد فوراً! 🚀</i>`;

        return ctx.reply(confirmMsg, {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [
              [{ text: '↩️ تراجع عن هذا التذكير وإلغاؤه فوراً', callback_data: `undo_action_${undoActionId}` }],
              [{ text: '🎯 عرض المهام والمواعيد', callback_data: 'menu_tasks' }, { text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }]
            ]
          }
        });
      }
    ...
    try {
      ...
      const isMedicalContent = /(?:ECG|STEMI|leads|artery|ventricle|atrium|CXR|X-ray|silhouette|cardiac|patient|occluded|heart|coronary|syndrome|aortic|pulmonary|valve|border|murmur|سؤال|الإجابة|المشتتات|التحشيشة|نيمونيك|تشخيص|موديول|CAD|PED)/i.test(textToParse);
      const isEngFluencySession = !isMedicalContent && (
        session?.mode === 'english_coach' ||
        /(?:Output transcript|fluency transcript|\bEN:\s*.*\bAR:)/i.test(textToParse)
      );
      let parsedResult;
  ...
  bot.on('poll_answer', async (ctx) => {
    try {
      ...
      const cleanTopic = (result.quiz?.topic || '').replace(/\[UID:\d+\]\s*/g, '').trim();
      const progressBar = '🟢'.repeat(result.nextLevel) + '⚪'.repeat(6 - result.nextLevel);
      const isMastered = result.nextLevel >= 6;

      let msg = '';
      if (result.isCorrect) {
        msg += `🎉 <b>إجابة ممتازة وصحيحة يا دكتور! 🩺✨</b>\n`;
        msg += `💎 <b>النقاط المكتسبة:</b> +30 نقطة خبرة طبيب\n`;
      } else {
        msg += `⚠️ <b>إجابة غير دقيقة يا دكتور.. لا بأس! 🩺</b>\n`;
        msg += `🔄 <b>التقييم:</b> تم إعادة جدولة السؤال للمراجعة العاجلة غداً لتثبيت وتصحيح المفهوم.\n`;
      }
      msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
      if (cleanTopic) {
        msg += `📚 <b>الموضوع:</b> <b>${cleanTopic}</b>\n`;
      }
      msg += `🏆 <b>المرحلة:</b> ${result.nextLevel} من 6 ${isMastered ? '👑 (تم الإتقان 100% مدى الحياة!)' : ''}\n`;
      msg += `📊 <b>مسار الإتقان:</b> ${progressBar}\n`;

      if (!isMastered && result.nextDate) {
        const nextReviewFormatted = new Date(result.nextDate).toLocaleDateString('ar-EG', { weekday: 'long', day: 'numeric', month: 'short' });
        msg += `⏰ <b>موعد المراجعة القادمة:</b> ${nextReviewFormatted}\n`;
      }

      msg += `━━━━━━━━━━━━━━━━━━━━━\n\n`;

      const fullExplanation = result.quiz?.explanation || result.quiz?.answer_and_explanation;
      if (fullExplanation) {
      ...
      const targetChatId = result.quiz?.chat_id || fromId;
  ...
  async function executeParsedLifeActions(ctx, result, fromId, passedSession = null) {
    ...
    // 💎 0.5 Record Medical Mnemonic Intent
    if (result.detected_type === 'record_mnemonic' || result.data?.medical_mnemonic || result.medical_mnemonic) {
      const m = result.data?.medical_mnemonic || result.medical_mnemonic || {};
      const course = (m.course_code || 'CAD402').toUpperCase();
      const topic = m.topic || 'تحشيشة طبية سريرية';
      const mText = m.mnemonic_text || result.mnemonic_text || '';
      const pearl = m.explanation_pearl || result.explanation_pearl || '';
      ...
      if (mText) {
        await saveMedicalMnemonic({
          ...
          topic,
          mnemonic_text: mText,
          explanation_pearl: pearl,
          source: 'voice_note'
        });

        let msg = `💎 <b>تم تسجيل التحشيشة الماسية في بنك النيمونيك بنجاح! 🧠✨</b>\n`;
        msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
        msg += `🏷️ <b>الموديول:</b> <code>[${course}]</code>\n`;
        msg += `📌 <b>الموضوع:</b> <b>${topic}</b>\n\n`;
        msg += `💡 <b>التحشيشة:</b>\n« <b>${mText}</b> »\n\n`;
        if (pearl) {
          msg += `🩺 <b>السريرية والـ Pearl:</b>\n${pearl}\n\n`;
        }
    ...
    // 💎 Auto-rescue Mnemonic if mentioned anywhere in summary or topic
    const summaryTextToCheck = result.summary_text || '';
    const hasMnemonicMention = /(?:تحشيش|نيمونيك|إسكيمة|mnemonic)/i.test(summaryTextToCheck);
    if (!result.data?.medical_mnemonic && hasMnemonicMention) {
      try {
        const topicMatch = summaryTextToCheck.match(/(?:عن|لموضوع|لموديول\s*[^عن]+عن)\s*([^(\n\r]+)/i);
        const mnemMatch = summaryTextToCheck.match(/(?:باستخدام|اختصار|تحشيشة|نيمونيك|إسكيمة)\s*[:«"'(]?\s*([^»"')\n\r]+)/i);
        const courseMatch = summaryTextToCheck.match(/(?:CAD\d*|PED\d*|RSD\d*|GIT\d*|NEURO\d*|[A-Z]{3,4}\d{3})/i);

        const course = (courseMatch ? courseMatch[0] : (data.study?.[0]?.course_code || 'CAD402')).toUpperCase();
        const topic = topicMatch ? topicMatch[1].trim() : (data.study?.[0]?.topic || 'تحشيشة سريرية');
        const mText = mnemMatch ? mnemMatch[1].trim() : (data.study?.[0]?.topic || 'تحشيشة مسجلة');
        ...
        await saveMedicalMnemonic({
          ...
          topic,
          mnemonic_text: mText,
          explanation_pearl: summaryTextToCheck,
          source: 'voice_note'
        });
      } catch (mErr) {
        console.warn('Auto-rescue mnemonic warning:', mErr.message);
      }
    }
    ...
    if (Array.isArray(data.english_flashcards) && data.english_flashcards.length > 0) {
      ...
      for (const card of data.english_flashcards) {
        try {
          ...
          if (term) {
            // Stagger next review times: 1st card in 15 mins, 2nd in 45 mins, etc.
            const staggerMins = cardIdx === 0 ? 15 : Math.min(24 * 60, 45 + cardIdx * 45);
            const nextReview = new Date(Date.now() + staggerMins * 60 * 1000).toISOString();
            ...
            const { data: row } = await supabase.from('english_spaced_flashcards').insert({
              ...
              usage_context: `[UID:${fromId}] ${category}`,
              repetition_level: 0,
              next_review_at: nextReview
            }).select('id').maybeSingle();
      ...
      if (savedCards.length > 0) {
        let engSummary = `🗣️ <b>بنك الطلاقة الإنجليزية (Spaced Repetition):</b> تم استخراج وتوثيق <b>(${savedCards.length} كارت وجملة بالكامل)</b> للتكرار المتباعد 🇬🇧✨\n`;
        const previewCards = savedCards.slice(0, 10);
        previewCards.forEach(c => {
          engSummary += `   • 🌟 <code>${c.term}</code> ➔ ${c.translation}\n`;
        });
        if (savedCards.length > 10) {
          engSummary += `   <i>...وتم حفظ (${savedCards.length - 10}) كروت أخرى ستصلك بمواعيدها للمراجعة!</i>\n`;
        }
        insertedSummary.push(engSummary.trim());
        keyboardRows.push([
          { text: `🧠 ابدأ مراجعة كارت الآن 🚀`, callback_data: 'review_eng_next' },
          { text: `🗣️ عرض كل كروت الإنجليزي (${savedCards.length}) 🇬🇧`, callback_data: 'menu_eng_spaced' }
        ]);
      }
    }

    // 2. Medical Quizzes (User-Driven)
    if (Array.isArray(data.medical_quizzes) && data.medical_quizzes.length > 0) {
      for (const q of data.medical_quizzes) {
        try {
          if (q.question) {
            const nextReview = new Date(Date.now() + 8 * 3600 * 1000).toISOString();
            const cleanTopic = q.topic || 'سؤال سريري للمراجعة والتثبيت';
            const courseCode = q.course_code || 'CAD402';

            let doctorPearl = q.doctor_pearl || null;
            if (q.options && Array.isArray(q.options) && q.options.length >= 2) {
              const metaObj = {
                options: q.options,
                correct_index: Number(q.correct_option_index || 0),
                explanation: q.answer_and_explanation || ''
              };
              doctorPearl = `<<<QUIZ_META_START>>>${JSON.stringify(metaObj)}<<<QUIZ_META_END>>> ${q.doctor_pearl || ''}`.trim();
            }

            const { data: row } = await supabase.from('medical_spaced_quizzes').insert({
              course_code: courseCode,
              topic: `[UID:${fromId}] [USER_INPUT] ${cleanTopic}`.trim(),
              question: q.question,
            ...
            if (row?.id) {
              recordedUndoItems.push({ table: 'medical_spaced_quizzes', id: row.id, summary: `🩺 كويز طبي [${courseCode}]` });
            }

            insertedSummary.push(`🩺 <b>سؤال وكويز للمراجعة [${courseCode}]:</b> ${cleanTopic}\n   └ ❓ <b>السؤال:</b> ${q.question.slice(0, 65)}...\n   └ 🧠 <b>تمت جدولته في التكرار المتباعد ليختبرك فيه البوت بانتظام! 🎯</b>`);
          }
        } catch (e) {
    ...
    // 2.1 Study Explanations (User-Driven Spaced Repetition)
    if (Array.isArray(data.study_explanations) && data.study_explanations.length > 0) {
      for (const exp of data.study_explanations) {
        try {
          const topic = exp.topic || 'شرح وتثبيت مفاهيم المنهج';
          const courseCode = exp.course_code || 'CAD402';
          const explText = exp.explanation_text || exp.text || '';
          const keyTakeaway = exp.key_takeaway || '';
          const nextReview = new Date(Date.now() + 8 * 3600 * 1000);

          const { data: row } = await supabase.from('academic_spaced_mastery').insert({
            telegram_id: fromId,
            course_code: courseCode,
            topic: `[USER_EXPLANATION] ${topic}`.trim(),
            repetition_stage: 1,
            mastery_pct: 25,
            mastery_status: 'تثبيت أولي',
            notes: `[usr:${fromId}] [EXPLANATION_TEXT:${encodeURIComponent(explText)}] [KEY_TAKEAWAY:${encodeURIComponent(keyTakeaway)}]`,
          ...
          if (row?.id) {
            await supabase.from('appointments_and_reminders').insert({
              title: `📖 مراجعة وتثبيت شرح: [${courseCode}] ${topic}`,
              due_datetime: nextReview.toISOString(),
              is_completed: false,
              is_notified: false,
              notes: `[usr:${fromId}] [academic_explanation:${row.id}] [course:${courseCode}]`
            });
            recordedUndoItems.push({ table: 'academic_spaced_mastery', id: row.id, summary: `📖 شرح [${topic}]` });
          }

          insertedSummary.push(
            `📖 <b>شرح وتثبيت متباعد [${courseCode}]:</b> <b>${topic}</b>\n` +
            (keyTakeaway ? `   ├ 💡 <b>الخلاصة:</b> ${keyTakeaway}\n` : '') +
            `   └ 🎯 <b>تم توثيق الشرح وجدولته في التكرار المتباعد ليتم إرساله لك على فترات حتى تثبيته وحفظه التام! 🧠✨</b>`
          );
        } catch (e) {
          console.warn('study_explanations insert error:', e.message);
        }
      }
    }
    ...
    // 9. Appointments & Reminders (Bridged & Resilient)
    data.appointments = Array.isArray(data.appointments) ? data.appointments : [];

    // 🌉 9.1 Bridge: Convert any tasks with due_datetime or reminder time to appointments
    if (Array.isArray(data.tasks) && data.tasks.length > 0) {
      for (const tk of data.tasks) {
        const hasDue = Boolean(tk.due_datetime || tk.reminder_after_minutes || tk.reminder_mins);
        const hasReminderWord = /تذكير|فكرني|ذكرني|نبهني/i.test(tk.title || '');
        if (hasDue || hasReminderWord) {
          let targetDt = tk.due_datetime;
          if (!targetDt && (tk.reminder_after_minutes || tk.reminder_mins)) {
            const m = Number(tk.reminder_after_minutes || tk.reminder_mins);
            targetDt = new Date(Date.now() + m * 60000).toISOString();
          }
          if (targetDt) {
            const cleanTitle = (tk.title || 'تذكير بمهمة').replace(/^تذكير\s*(?:بـ|ب)?\s*/i, '').trim();
            if (!data.appointments.some(a => a.title === cleanTitle || a.title === tk.title)) {
              data.appointments.push({
                title: cleanTitle,
                due_datetime: targetDt,
                remind_at: targetDt,
                notes: tk.notes || `تذكير بمهمة: ${cleanTitle}`
              });
            }
          }
        }
    ...
    // 🛡️ 9.2 Fallback: If appointments still empty, extract from raw user text or summary_text
    if (data.appointments.length === 0) {
      const rawText = String(ctx.message?.text || ctx.message?.caption || result.raw_transcription || '').trim();
      const extracted = parseArabicReminderInstant(rawText) || parseArabicReminderInstant(result.summary_text || '');
      if (extracted) {
        data.appointments.push({
          title: extracted.title,
          due_datetime: extracted.dueDatetime,
          remind_at: extracted.dueDatetime,
          notes: `تذكير ذكي مستخرج تلقائياً`
        });
      }
    }
        ...
        try {
          ...
          if (apptTitle && dueDt) {
            ...
            if (row?.id) {
              ...
              // Fast-track immediate timer for short delays (<= 120 minutes / 2 hours)
  ...
  bot.action(/^launch_pdf_quiz_(.+?)_(.*)$/, async (ctx) => {
    ...
    try {
      ...
      const safeCorrectIdx = Math.max(0, Math.min(options.length - 1, correctIdx));
      const pollExplanation = formatNativePollExplanation(mcq.explanation, options[safeCorrectIdx]);
  ...
  // 📖 User Study Explanation SRS Action Handlers
  bot.action(/^ack_expl_pass_([^_]+)_(.+)$/, async (ctx) => {
    const masteryId = ctx.match[1];
    const apptId = ctx.match[2];
    await ctx.answerCbQuery('🌟 عاش يا دكتور! تم ترقية مرحلة تثبيت الشرح');
    ...
    try {
      ...
      const nextStage = Math.min(6, (row.repetition_stage || 1) + 1);
      ...
      if (nextStage < 6) {
        await supabase.from('appointments_and_reminders').insert({
          title: `📖 مراجعة وتثبيت شرح: [${row.course_code}] ${(row.topic || '').replace(/\[USER_EXPLANATION\]\s*/, '')}`,
          due_datetime: nextDate.toISOString(),
          is_completed: false,
          is_notified: false,
          notes: `[usr:${ctx.from.id}] [academic_explanation:${row.id}] [course:${row.course_code}] مرحلة: ${nextStage}/6`
        });
      }

      await addDoctorXp(ctx.from.id, 30);

      const nextDateFormatted = nextDate.toLocaleDateString('ar-EG', { weekday: 'long', day: 'numeric', month: 'short' });
      return ctx.editMessageText(
        `✅ <b>عاش يا دكتور! تم تثبيت الشرح وترقية مرحلة الحفظ 🧠✨</b>\n━━━━━━━━━━━━━━━━━━━━━\n` +
        `📌 <b>الموضوع:</b> [${row.course_code}] ${(row.topic || '').replace(/\[USER_EXPLANATION\]\s*/, '')}\n` +
        `🏆 <b>المرحلة:</b> ${nextStage}/6 (نسبة الإتقان: <b>${pcts[nextStage]}%</b>)\n` +
        (nextStage >= 6
          ? `👑 <b>تهانينا! تم إتقان هذا الشرح بنسبة 100% ورسخ في الذاكرة طويلة المدى! 🎯</b>`
          : `⏰ <b>المراجعة القادمة:</b> ${nextDateFormatted} <i>(مجداول تلقائياً بالتكرار المتباعد)</i>`),
        { parse_mode: 'HTML' }
      ).catch(() => {});
    } catch (e) {
      console.warn('ack_expl_pass error:', e.message);
    }
  });

  bot.action(/^ack_expl_retry_([^_]+)_(.+)$/, async (ctx) => {
    const masteryId = ctx.match[1];
    const apptId = ctx.match[2];
    await ctx.answerCbQuery('🔄 تم تحديد موعد مراجعة قريب لتثبيته');
    ...
    try {
      ...
      const retryDate = new Date(Date.now() + 4 * 3600 * 1000);
      await supabase.from('academic_spaced_mastery').update({
        next_review_at: retryDate.toISOString(),
        last_reviewed_at: new Date().toISOString()
      }).eq('id', masteryId);

      await supabase.from('appointments_and_reminders').insert({
        title: `📖 مراجعة وتثبيت شرح: [${row.course_code}] ${(row.topic || '').replace(/\[USER_EXPLANATION\]\s*/, '')}`,
        due_datetime: retryDate.toISOString(),
        is_completed: false,
        is_notified: false,
        notes: `[usr:${ctx.from.id}] [academic_explanation:${row.id}] [course:${row.course_code}] إعادة تثبيت سريعة`
      });

      return ctx.editMessageText(
        `🔄 <b>ولا تشيل هم يا دكتور!</b>\n` +
        `تمت إعادة جدولة هذا الجزء بعد <b>4 ساعات</b> لتكراره وتثبيته في عقلك حتى يستقر تماماً 🧠💪`,
        { parse_mode: 'HTML' }
      ).catch(() => {});
    } catch (e) {}
  });

  bot.action(/^ack_expl_master_([^_]+)_(.+)$/, async (ctx) => {
    const masteryId = ctx.match[1];
    const apptId = ctx.match[2];
    await ctx.answerCbQuery('👑 تم تعليمه كمتقن نهائياً 100%!');
    ...
    try {
      await supabase.from('academic_spaced_mastery').update({
        repetition_stage: 6,
        mastery_pct: 100,
        mastery_status: 'متقن راسخ',
        last_reviewed_at: new Date().toISOString()
      }).eq('id', masteryId);

      await addDoctorXp(ctx.from.id, 50);

      return ctx.editMessageText(
        `👑 <b>عاش يا دكتور! تم نقل هذا الشرح إلى (المتقن الراسخ 100%) بنجاح 🎯✨</b>`,
        { parse_mode: 'HTML' }
      ).catch(() => {});
    } catch (e) {}
  });
  ...
  bot.action(/^reveal_eng_(.+)$/, async (ctx) => {
    ...
    const lvl = card.repetition_level || 0;
    const progressBar = '🟢'.repeat(lvl) + '⚪'.repeat(6 - lvl);

    let msg = `🇬🇧 <b>ENGLISH FLASHCARD • الترجمة والشرح 🧠</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `🌟 <b>${card.term_or_sentence}</b>\n\n`;
    msg += `🇪🇬 <b>الترجمة والمعنى بالمصري:</b>\n<b>${card.egyptian_translation}</b>\n`;

    if (card.example_sentence) {
      msg += `\n📝 <b>مثال سياقي:</b>\n<i>"${card.example_sentence}"</i>\n`;
    }

    msg += `\n📊 <b>المرحلة الحالية:</b> [${lvl}/6] [${progressBar}]\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `👇 <i>كيف كان استرجاعك للمصطلح؟</i>`;

    const keyboard = {
      inline_keyboard: [
        [
          { text: '🟢 حفظتها تمام (Level Up)', callback_data: `pass_eng_${card.id}_${lvl}` },
          { text: '🔴 نسيتها (إعادة تثبيت)', callback_data: `fail_eng_${card.id}` }
        ],
        [
          { text: '👑 إتقان نهائي 100% (إيقاف للأبد)', callback_data: `master_eng_${card.id}` }
        ]
      ]
    };
  ...
  bot.action(/^master_eng_(.+)$/, async (ctx) => {
    const cardId = ctx.match[1];
    
    // 1. Fetch card details to keep the text visible
    const { data: card } = await supabase.from('english_spaced_flashcards').select('*').eq('id', cardId).maybeSingle();

    // 2. Mark card as mastered and set next_review_at to year 2099 so it NEVER appears again
    ...
    // 3. Also mark any similar / duplicate flashcards as mastered so they NEVER re-appear!
    if (card?.term_or_sentence) {
      const cleanTerm = card.term_or_sentence.trim();
      const first25 = cleanTerm.slice(0, 25);
      await supabase.from('english_spaced_flashcards').update({
        repetition_level: 6,
        is_mastered: true,
        next_review_at: '2099-01-01T00:00:00Z',
        last_reviewed_at: new Date().toISOString()
      }).ilike('term_or_sentence', `%${first25}%`).neq('id', cardId);
    }

    await ctx.answerCbQuery('👑 تم نقل الكارت للمتقن نهائياً!');

    let msg = `🇬🇧 <b>ENGLISH FLASHCARD • كارت متقن 100% 👑</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    if (card) {
      msg += `🌟 <b>${card.term_or_sentence}</b>\n\n`;
      msg += `🇪🇬 <b>الترجمة والمعنى بالمصري:</b>\n<b>${card.egyptian_translation || 'مصطلح إنجليزي'}</b>\n`;
      if (card.example_sentence) {
        msg += `\n📝 <b>مثال سياقي:</b>\n<i>"${card.example_sentence}"</i>\n`;
      }
      msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    }
    msg += `👑 <b>عاش يا دكتور! تم نقل الكارت إلى (المتقن نهائياً 100%) ولن يتكرر إرساله مجدداً! 🎯✨</b>\n`;
    msg += `📊 <b>التقدم العصبي:</b> [🟢🟢🟢🟢🟢🟢] (مكتمل)\n`;

    return ctx.editMessageText(msg, {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
          [{ text: '➡️ الكارت التالي ⏭️', callback_data: 'review_eng_next' }],
          [{ text: '📚 بنك الكروت', callback_data: 'menu_eng_spaced' }]
        ]
      }
    }).catch(() => {});
  });

  bot.action('mute_eng_spaced', async (ctx) => {
    const fromId = ctx.from?.id;
    await ctx.answerCbQuery('تم إيقاف الإشعارات التلقائية');
    const prof = await getUserProfile(fromId);
    const prefs = prof?.preferences || {};
    prefs.english_auto_review = false;
    await updateUserPreferences(fromId, prefs);
    return ctx.editMessageText(
      '⏸️ <b>تم إيقاف إرسال إشعارات فلاش كاردز الإنجليزية التلقائية! 🇬🇧</b>\n━━━━━━━━━━━━━━━━━━━━━\n' +
      'لن يرسل لك البوت أي كروت فجأة بعد الآن.\n\n' +
      '💡 <i>يمكنك مراجعة كافة كروتك في أي وقت بالدخول إلى قسم "فلاش كاردز الإنجليزية" في لوحة التحكم، أو بكتابة أمر /english في الشات!</i> ✨',
      {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: [
  ...
  bot.action(/^pass_eng_(.+?)_(\d+)$/, async (ctx) => {
    ...
    await supabase.from('english_spaced_flashcards').update({
      ...
      next_review_at: isMastered ? '2099-01-01T00:00:00Z' : nextReviewDate,
      is_mastered: isMastered,
      last_reviewed_at: new Date().toISOString()
    }).eq('id', cardId);

    if (isMastered && card?.term_or_sentence) {
      const first25 = card.term_or_sentence.trim().slice(0, 25);
      await supabase.from('english_spaced_flashcards').update({
        repetition_level: 6,
        is_mastered: true,
        next_review_at: '2099-01-01T00:00:00Z',
    ...
    const progressBar = '🟢'.repeat(nextLevel) + '⚪'.repeat(6 - nextLevel);
    const nextReviewFormatted = new Date(nextReviewDate).toLocaleDateString('ar-EG', { weekday: 'long', day: 'numeric', month: 'short' });

    let msg = `🇬🇧 <b>ENGLISH FLASHCARD • تم الحفظ بنجاح 🟢</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    if (card) {
      msg += `🌟 <b>${card.term_or_sentence}</b>\n\n`;
      msg += `🇪🇬 <b>الترجمة والمعنى بالمصري:</b>\n<b>${card.egyptian_translation || 'مصطلح إنجليزي'}</b>\n`;
    ...
    msg += `🎉 <b>إتقان ممتاز يا دكتور! 🇬🇧✨ (+20 English XP)</b>\n`;
    msg += `🏆 <b>المرحلة:</b> [${nextLevel}/6] ${isMastered ? '👑 (تم الإتقان 100% مدى الحياة!)' : ''}\n`;
    msg += `📊 <b>التقدم العصبي:</b> [${progressBar}]\n`;
    if (!isMastered) {
      msg += `⏰ <b>المراجعة القادمة:</b> ${nextReviewFormatted}\n`;
    }
  ...
  bot.action(/^fail_eng_(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery('تم جدولتها بعد 24 ساعة لتثبيتها');

    let msg = `🇬🇧 <b>ENGLISH FLASHCARD • إعادة التثبيت 🔄</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    if (card) {
      msg += `🌟 <b>${card.term_or_sentence}</b>\n\n`;
      msg += `🇪🇬 <b>الترجمة والمعنى بالمصري:</b>\n<b>${card.egyptian_translation || 'مصطلح إنجليزي'}</b>\n`;
    ...
    msg += `🔄 <b>تمت إعادة ضبط الكلمة لتثبيتها يا دكتور! 🧠</b>\n`;
    msg += `📊 <b>المرحلة:</b> [0/6] — إعادة التثبيت غداً لتصحيح المعنى!\n`;
    msg += `⏰ <b>المراجعة القادمة:</b> غداً بإذن الله\n`;
    ...
    return ctx.editMessageText(msg, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '➡️ الكارت التالي ⏭️', callback_data: 'review_eng_next' }],
          [{ text: '📚 استعراض بنك الكروت', callback_data: 'menu_eng_spaced' }]
        ]
      }
    }).catch(() => {});
  });

  // 🎯 Interactive On-Demand English Flashcard Review Service
  async function serveInteractiveEnglishCard(ctx, fromId, edit = false) {
    let query = supabase
      .from('english_spaced_flashcards')
      .select('*')
      .eq('is_mastered', false)
      .lt('repetition_level', 6)
      .order('next_review_at', { ascending: true })
      .limit(40);

    if (Number(fromId) !== ADMIN_CHAT_ID) {
      query = query.ilike('usage_context', `%[UID:${fromId}]%`);
    }

    const { data: rows, error } = await query;
    if (error || !rows || rows.length === 0) {
      const emptyMsg = `🎉 <b>ما شاء الله يا دكتور! لا توجد كروت مستحقة للمراجعة الآن! 🇬🇧✨</b>\n━━━━━━━━━━━━━━━━━━━━━\nكافة كروتك إما متقنة 100% أو مثبتة بمواعيدها القادمة.`;
      const kb = { inline_keyboard: [[{ text: '📚 استعراض كل الكروت', callback_data: 'menu_eng_spaced' }]] };
      if (edit) return ctx.editMessageText(emptyMsg, { parse_mode: 'HTML', reply_markup: kb }).catch(() => {});
      return ctx.reply(emptyMsg, { parse_mode: 'HTML', reply_markup: kb });
    }

    const userCards = rows.filter(c => {
      const ctxStr = c.usage_context || '';
      if (Number(fromId) === ADMIN_CHAT_ID) {
        return ctxStr.includes(`[UID:${fromId}]`) || !ctxStr.includes('[UID:');
      }
      return ctxStr.includes(`[UID:${fromId}]`);
    });

    // Deduplicate cards so near-identical sentences are never shown consecutively
    const seenSentences = new Set();
    const uniqueUserCards = [];
    userCards.forEach(c => {
      const sKey = (c.term_or_sentence || '').toLowerCase().replace(/[^\w\u0600-\u06FF]/g, '').slice(0, 30);
      if (sKey && !seenSentences.has(sKey)) {
        seenSentences.add(sKey);
        uniqueUserCards.push(c);
      }
    });

    if (uniqueUserCards.length === 0) {
      const noCardsMsg = `✨ <i>لا توجد كروت مجدولة في حسابك حالياً. أرسل أي جملة أو محادثة وسيتم حفظها وجدولتها فوراً!</i>`;
      if (edit) return ctx.editMessageText(noCardsMsg, { parse_mode: 'HTML' }).catch(() => {});
      return ctx.reply(noCardsMsg, { parse_mode: 'HTML' });
    }

    const card = uniqueUserCards[0];
    const lvl = card.repetition_level || 0;
    const progressBar = '🟢'.repeat(lvl) + '⚪'.repeat(6 - lvl);
    let msg = `🇬🇧 <b>ENGLISH FLASHCARD • مراجعة المصطلحات 🧠</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `🌟 <b>${card.term_or_sentence}</b>\n\n`;
    msg += `📊 المرحلة: <b>${lvl}/6</b> [${progressBar}]\n`;
    msg += `💡 <i>فكر في المعنى والاستخدام السريري، ثم اضغط لإظهار الترجمة:</i>`;

    const keyboard = {
      inline_keyboard: [
        [{ text: '💡 أظهر الترجمة والشرح بالمصري 🇪🇬', callback_data: `reveal_eng_${card.id}` }],
        [
          { text: '⏭️ كارت آخر', callback_data: `skip_eng_${card.id}` },
          { text: '📚 بنك الكلمات', callback_data: 'menu_eng_spaced' }
        ]
      ]
    };
  ...
  bot.action('review_eng_next', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    return serveInteractiveEnglishCard(ctx, ctx.from?.id, true);
  });

  bot.action(/^skip_eng_(.+)$/, async (ctx) => {
    await ctx.answerCbQuery('تم التخطي مؤقتاً').catch(() => {});
    const cardId = ctx.match[1];
    await supabase.from('english_spaced_flashcards').update({
  ...
  bot.command(['review_eng', 'flashcard', 'flashcards', 'بطاقة', 'كارت'], async (ctx) => {
    return serveInteractiveEnglishCard(ctx, ctx.from?.id, false);
  });
  ...
  bot.action(/^reveal_med_(.+)$/, async (ctx) => {
    ...
    const lvl = q.repetition_level || 0;
    const progressBar = '🟢'.repeat(lvl) + '⚪'.repeat(6 - lvl);

    let msg = `🩺 <b>سؤال سريري وتثبيت معرفي [${q.course_code || 'MED'}]:</b>\n━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `❓ <b>السؤال:</b>\n${q.question}\n\n`;
    msg += `💡 <b>الإجابة والشرح:</b>\n${q.answer_and_explanation}\n`;
    const cleanPearl = (q.doctor_pearl || '').replace(/<<<QUIZ_META_START>>>[\s\S]*?<<<QUIZ_META_END>>>/g, '').trim();
    if (cleanPearl) msg += `\n🔬 <b>تريكة الراوند والشرح بالمصري:</b>\n${cleanPearl}\n`;
    msg += `\n📊 <b>المرحلة:</b> [${lvl}/6] [${progressBar}]`;

    const keyboard = {
      inline_keyboard: [
        [
          { text: '🟢 متقنة 100% (Level Up)', callback_data: `pass_med_${q.id}_${lvl}` },
  ...
  bot.action(/^pass_med_(.+?)_(\d+)$/, async (ctx) => {
    ...
    let msg = `🎯 <b>إتقان طبي سريري ممتاز يا دكتور! 🩺✨ (+30 Doctor XP)</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `🏆 <b>المرحلة:</b> [${nextLevel}/6] ${isMastered ? '👑 (تم الإتقان 100% مدى الحياة!)' : ''}\n`;
    msg += `📊 <b>التقدم العصبي:</b> [${progressBar}]\n`;
    if (!isMastered) {
    ...
    return ctx.editMessageText(msg, { parse_mode: 'HTML' }).catch(() => {});
  ...
  bot.action(/^fail_med_(.+)$/, async (ctx) => {
    ...
    let msg = `🔄 <b>تمت إعادة جدولة السؤال لتثبيته في الذاكرة! 🩺</b>\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `📊 <b>المرحلة:</b> [0/6] — إعادة التثبيت غداً لتصحيح المفهوم!\n`;
    msg += `⏰ <b>المراجعة القادمة:</b> غداً بإذن الله\n`;
  ...
  bot.command(['english', 'eng', 'انجليزي', 'إنجليزي', 'vocab'], async (ctx) => {
    const fromId = ctx.from?.id;
    let query = supabase.from('english_spaced_flashcards').select('*').order('created_at', { ascending: false }).limit(25);
    if (Number(fromId) !== ADMIN_CHAT_ID) {
      query = query.ilike('usage_context', `%[UID:${fromId}]%`);
    }
    const { data: allRows } = await query;
    const cards = (allRows || []).filter(c => {
      const ctxStr = c.usage_context || '';
      if (Number(fromId) === ADMIN_CHAT_ID) {
        return ctxStr.includes(`[UID:${fromId}]`) || !ctxStr.includes('[UID:');
      }
    ...
    if (!cards || cards.length === 0) {
      ...
      cards.slice(0, 15).forEach((c, idx) => {
        msg += `${idx + 1}. 🌟 <b>${c.term_or_sentence}</b>\n   ➔ ${c.egyptian_translation} <i>[مستوى: ${c.repetition_level || 0}/6]</i>\n\n`;
      });
    }
    return ctx.reply(msg, {
      parse_mode: 'HTML',
  ...
  bot.action('menu_eng_spaced', async (ctx) => {
    ...
    if (!cards || cards.length === 0) {
      msg += `✨ <i>لا توجد كلمات محفوظة بعد. أرسل أي كلمة أو جملة وسيترجمها البوت بالمصري ويجدولها للمراجعة!</i>\n`;
    } else {
      cards.forEach((c, idx) => msg += `${idx + 1}. 🌟 <b>${c.term_or_sentence}</b>\n   ➔ ${c.egyptian_translation} <i>[مستوى: ${c.repetition_level || 0}/6]</i>\n\n`);
    }
  ...
  // 💎 31. Mnemonics & Pearls Interactive Actions
  bot.action('menu_mnemonics', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const fromId = ctx.from?.id;
    return sendMnemonicsViewer(ctx, fromId, 'ALL', 0, true);
  });

  bot.action(/^mnem_nav_(\d+)_(.+)$/, async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const fromId = ctx.from?.id;
    const pageIdx = parseInt(ctx.match[1], 10);
    const filter = ctx.match[2];
    return sendMnemonicsViewer(ctx, fromId, filter, pageIdx, true);
  });

  bot.action(/^mnem_filter_([^_]+)_(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const fromId = ctx.from?.id;
    const filter = ctx.match[1];
    return sendMnemonicsViewer(ctx, fromId, filter, 0, true);
  });

  bot.action(/^mnem_random_(.+)$/, async (ctx) => {
    await ctx.answerCbQuery('🎲 اختيار تحشيشة عشوائية!').catch(() => {});
    const fromId = ctx.from?.id;
    const filter = ctx.match[1];
    const list = await getMedicalMnemonics(fromId, filter);
    if (!list || list.length === 0) return sendMnemonicsViewer(ctx, fromId, filter, 0, true);
    const randomIdx = Math.floor(Math.random() * list.length);
    return sendMnemonicsViewer(ctx, fromId, filter, randomIdx, true);
  });

  bot.action('record_mnemonic_prompt', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const fromId = ctx.from?.id;
    await setUserSession(fromId, { state: 'waiting_mnemonic_voice' });

    let promptMsg = `🎙️ <b>تسجيل تحشيشة ونيمونيك جديدة بالصوت أو النص 🧠✨</b>\n`;
    promptMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    promptMsg += `سجل فويس الآن بصوتك وقول التحشيشة أو اكتبها في رسالة بالأسفل:\n\n`;
    promptMsg += `💡 <b>أمثلة:</b>\n`;
    promptMsg += `• <i>«سجل تحشيشة لتضخم الأذين الأيسر: الأذين الأيسر كئيب بـ 2 بيعمل ظل مضاعف والكارينا أكبر من 90»</i>\n`;
    promptMsg += `• <i>«نيمونيك النترات والفياجرا: النترات مع الفياجرا مقبرة»</i>\n`;
    promptMsg += `• أو اذكر التحشيشة والـ Pearl على طول.\n\n`;
    promptMsg += `✨ <i>سيقوم البوت بفرزها وتصنيف الموديول وحفظها في بنكك فوراً! 🚀</i>`;

    const cancelKb = {
      inline_keyboard: [
        [{ text: '❌ إلغاء والعودة للبنك', callback_data: 'menu_mnemonics' }]
      ]
    };

    return ctx.reply(promptMsg, { parse_mode: 'HTML', reply_markup: cancelKb });
  });

  // ⚡ 32. Distractor Quiz Generator (Adversarial Retesting)
  bot.action(/^quiz_distractors_(.+)$/, async (ctx) => {
    await ctx.answerCbQuery('🎯 جاري توليد كويز المشتتات...').catch(() => {});
    const fromId = ctx.from?.id;
    const vaultId = ctx.match[1];

    let vaultItem = null;
    try {
      const { data } = await supabase.from('medical_error_vault').select('*').eq('id', vaultId).maybeSingle();
      vaultItem = data;
    } catch (_) {}

    if (!vaultItem) {
      try {
        const { data: fbData } = await supabase.from('thoughts_and_wisdom').select('content').eq('category', 'سجل أخطاء الأسئلة الطبية').order('created_at', { ascending: false }).limit(5);
        if (Array.isArray(fbData)) {
          for (const row of fbData) {
            const p = JSON.parse(row.content);
            if (p.id === vaultId || vaultId === 'last') {
              vaultItem = p;
              break;
            }
          }
    ...
    if (!vaultItem || !Array.isArray(vaultItem.distractors) || vaultItem.distractors.length === 0) {
      return ctx.reply('⚠️ لم يتم العثور على مشتتات لهذا السؤال، يمكنك مراجعة الأسئلة عبر بنك الكويزات الطبية.');
    }

    const distractor = vaultItem.distractors[Math.floor(Math.random() * vaultItem.distractors.length)];
    const optName = typeof distractor === 'string' ? distractor : distractor.option;
    const optReason = typeof distractor === 'string' ? '' : distractor.reason;

    let quizMsg = `⚡ <b>اختبار المشتتات والأفخاخ الذكي (Distractor Hunter) 🎯</b>\n`;
    quizMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    quizMsg += `📌 <b>سؤال مضاد لاختبار ثبات الفهم:</b>\n`;
    quizMsg += `في السؤال السابق ظهر الخيار المشتت التالي:\n`;
    quizMsg += `« <b>${optName}</b> »\n\n`;
    quizMsg += `❓ <b>لماذا هذا الخيار كان خاطئاً وما هو تفسيره السريري الصحيح؟</b>\n\n`;
    quizMsg += `💡 <i>فكر في الإجابة قبل الضغط على زر كشف التفسير:</i>`;

    const distractorKb = {
      inline_keyboard: [
        [{ text: '🔍 كشف التفسير السريري الصحيح', callback_data: `reveal_distractor_${encodeURIComponent(optName.slice(0, 30))}_${encodeURIComponent(optReason.slice(0, 80))}` }],
        [{ text: '💎 فتح بنك التحشيشات والـ Pearls', callback_data: 'menu_mnemonics' }],
        [{ text: '🚀 القائمة الرئيسية', callback_data: 'menu_main' }]
      ]
    };

    return ctx.reply(quizMsg, { parse_mode: 'HTML', reply_markup: distractorKb });
  });

  bot.action(/^reveal_distractor_(.*)$/, async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const parts = ctx.match[1].split('_');
    const optName = decodeURIComponent(parts[0] || 'الخيار المشتت');
    const optReason = decodeURIComponent(parts.slice(1).join('_') || 'سبب إكلينيكي خاص بحالة أخرى');

    let replyMsg = `✅ <b>تفسير المشتت السريري الصحيح:</b>\n`;
    replyMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    replyMsg += `📌 <b>الخيار:</b> <b>${optName}</b>\n`;
    replyMsg += `🩺 <b>التفسير والربط السريري:</b>\n${optReason}\n\n`;
    replyMsg += `🎯 <i>عاش يا دكتور! كده قفلت ثغرة الخيار ده تماماً ومش هتتلخبط فيه في أي امتحان! 🌟</i>`;
  ...
  bot.action(/^ack_read_sharia_(.+)$/, async (ctx) => {
    ...
    const { addDoctorXp } = await import('./supabase.js');
    await addDoctorXp(fromId, 20);

    await ctx.answerCbQuery('✅ تم تأكيد القراءة والاستيعاب بنجاح! (+20 XP)', { show_alert: true }).catch(() => {});

    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم استيعاب الدرس 📖✨', callback_data: 'noop_read' }],
        [{ text: '📖 كبسولة أخرى', callback_data: `get_sharia_capsule_${capsule.id}` }]
      ]
    }).catch(() => {});
  });
  ...
  bot.action(/^ack_read_discipline_(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery('✅ تم توثيق استيعاب المبدأ العلمي! (+20 XP)\n⏳ تمت جدولة كويز التثبيت ليختبرك البوت فيه بعد ساعتين (+2h) 🎯', { show_alert: true }).catch(() => {});

    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم استيعاب المبدأ العلمي — مجدول للمراجعة بعد ساعتين (+2h) ⏳', callback_data: 'noop_read' }],
  ...
  bot.action(/^ack_read_ayah_(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery('✅ تم توثيق تدبر الآية وسبب النزول! (+20 XP)\n⏳ تمت جدولة سؤال الكويز ليختبرك فيه البوت بعد ساعتين (+2h) 🎯', { show_alert: true }).catch(() => {});

    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم تدبر الآية — مجدولة للمراجعة بعد ساعتين (+2h) ⏳', callback_data: 'noop_read' }],
  ...
  bot.action(/^ack_read_bukhari_(.+)$/, async (ctx) => {
    ...
    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم استيعاب الحديث والدرس القيادي — مجدول للمراجعة بعد ساعتين (+2h) ⏳', callback_data: 'noop_read' }],
  ...
  bot.action(/^ack_read_prophet_(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery('✅ تم توثيق الموقف القيادي النبوي! (+20 XP)\n⏳ سيتم إرسال كويز الموقف بعد ساعتين (+2h) 🎯', { show_alert: true }).catch(() => {});

    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم استيعاب الموقف القيادي — مجدول للمراجعة بعد ساعتين (+2h) ⏳', callback_data: 'noop_read' }],
  ...
  bot.action(/^ack_read_sahabi_(.+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery('✅ تم توثيق مناقب الصحابي! (+20 XP)\n⏳ سيصلك كويز الصحابي لاختبار حفظك بعد ساعتين (+2h) 🎯', { show_alert: true }).catch(() => {});

    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم استيعاب سيرة الصحابي — مجدول للمراجعة بعد ساعتين (+2h) ⏳', callback_data: 'noop_read' }],
        [{ text: '🌟 صحابي آخر', callback_data: 'get_sahabi_spotlight' }]
      ]
    }).catch(() => {});
  });

  bot.action(/^ack_read_calm(?:_([a-zA-Z0-9_]+))?(?:_(.+))?$/, async (ctx) => {
    const fromId = ctx.from?.id;
    const category = ctx.match?.[1];
    const itemId = ctx.match?.[2];

    if (fromId) {
      const { addDoctorXp, scheduleAdminSpacedQuiz } = await import('./supabase.js');
      await addDoctorXp(fromId, 20, 'daily_pearl_read').catch(() => {});

      // 🧠 Spaced Repetition Scheduling: Schedule interactive quiz 7 days later for long-term consolidation
      if (category && itemId) {
        try {
          let quiz = null;
          let topic = 'تثبيت المفهوم';
          let courseCode = category.toUpperCase();

          if (category === 'sharia') {
            const { SHARIA_CAPSULES } = await import('./sharia_sciences.js');
            const item = SHARIA_CAPSULES.find(c => c.id === itemId);
            if (item?.quiz) { quiz = item.quiz; topic = item.title; courseCode = 'SHARIA'; }
          } else if (category === 'past') {
            const { PAST_RULERS_MISTAKES } = await import('./statesman_engine.js');
            const item = PAST_RULERS_MISTAKES.find(p => p.id === itemId);
            if (item?.quiz) { quiz = item.quiz; topic = item.ruler_or_dynasty || item.lesson_title; courseCode = 'STATESMAN'; }
          } else if (category === 'sahaba') {
            const { SAHABA_SPOTLIGHTS } = await import('./admin_curriculum.js');
            const item = SAHABA_SPOTLIGHTS.find(s => s.id === itemId);
            if (item?.quiz) { quiz = item.quiz; topic = item.name; courseCode = 'SAHABA_SPOTLIGHT'; }
          } else if (category === 'bukhari') {
            const { BUKHARI_HADITHS } = await import('./admin_curriculum.js');
            const item = BUKHARI_HADITHS.find(b => b.id === itemId);
            if (item?.quiz) { quiz = item.quiz; topic = item.hadith_title; courseCode = 'BUKHARI'; }
          } else if (category === 'discipline') {
            const { SCIENTIFIC_DISCIPLINE_INSIGHTS } = await import('./admin_curriculum.js');
            const item = SCIENTIFIC_DISCIPLINE_INSIGHTS.find(d => d.id === itemId);
            if (item?.quiz) { quiz = item.quiz; topic = item.title; courseCode = 'DISCIPLINE'; }
          } else if (category === 'prophet') {
            const { PROPHETIC_SITUATIONS } = await import('./admin_curriculum.js');
            const item = PROPHETIC_SITUATIONS.find(p => p.id === itemId);
            if (item?.quiz) { quiz = item.quiz; topic = item.title; courseCode = 'PROPHETIC_LEADERSHIP'; }
          } else if (category === 'ayah') {
            const { QURAN_AYAHS_WITH_ASBAB } = await import('./admin_curriculum.js');
            const item = QURAN_AYAHS_WITH_ASBAB.find(a => a.id === itemId);
            if (item?.quiz) { quiz = item.quiz; topic = item.surah; courseCode = 'QURAN_ASBAB'; }
          }
          ...
          if (quiz) {
            await scheduleAdminSpacedQuiz(fromId, {
              ...
              correct_option_index: quiz.correct_option_index || 0,
              explanation: quiz.explanation || '',
              delayDays: 7
            });
          }
        } catch (err) {
          console.warn('[ack_read_calm quiz schedule error]:', err.message);
        }
      }
    }

    await ctx.answerCbQuery('✅ بارك الله فيك ونفعك بما علمت! (+20 XP)\n⏳ تمت جدولة سؤال التثبيت الأسبوعي ليختبرك فيه البوت بعد أسبوع (7d SRS 🧠✨)', { show_alert: true }).catch(() => {});
    return ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [{ text: '✅ تم الاستيعاب والتدبر — مجدول بعد أسبوع (7d SRS) 🧠', callback_data: 'noop_read' }]
      ]
    }).catch(() => {});
  });

  bot.action('noop_read', async (ctx) => {
    return ctx.answerCbQuery('📌 تم تأكيد استيعابك لهذا الدرس مسبقاً!').catch(() => {});
  });
  ...
  bot.action(/^take_sharia_quiz(?:_(.+))?$/, async (ctx) => {
    ...
    try {
      ...
      const shuffled = shuffleQuizOptions(capsule.quiz.options, capsule.quiz.correct_option_index || 0);
      ...
      await saveNativeQuizPoll(fromId, {
        ...
        correct_option_index: shuffled.correctIndex,
        explanation: capsule.quiz.explanation,
        repetition_level: 1
      });
  ...
  bot.action('take_statesman_quiz', async (ctx) => {
    ...
    try {
      // 🎲 Shuffle options so correct answer position varies randomly
      const shuffled = shuffleQuizOptions(mistake.quiz.options, mistake.quiz.correct_option_index || 0);
      const pollMsg = await bot.telegram.sendQuiz(
        fromId,
        ...
        shuffled.options,
        {
          correct_option_id: shuffled.correctIndex,
          is_anonymous: false,
          explanation: formatNativePollExplanation(mistake.quiz.explanation, shuffled.options[shuffled.correctIndex])
        }
      );
      await saveNativeQuizPoll(fromId, {
  ...
  bot.action('take_purity_quiz', async (ctx) => {
    ...
    try {
      // 🎲 Shuffle options so correct answer position varies randomly
      const shuffled = shuffleQuizOptions(fuel.quiz.options, fuel.quiz.correct_option_index || 0);
      const pollMsg = await bot.telegram.sendQuiz(
        fromId,
        fuel.quiz.question,
        shuffled.options,
        {
          correct_option_id: shuffled.correctIndex,
          is_anonymous: false,
          explanation: formatNativePollExplanation(fuel.quiz.explanation, shuffled.options[shuffled.correctIndex])
        }
      );
      await saveNativeQuizPoll(fromId, {
        telegram_poll_id: pollMsg.poll.id,
  ...
  bot.action(/^take_discipline_quiz(?:_(.+))?$/, async (ctx) => {
    ...
    try {
      // 🎲 Shuffle options so correct answer position varies randomly
      const shuffled = shuffleQuizOptions(quiz.options, quiz.correct_option_index || 0);
      const pollMsg = await bot.telegram.sendQuiz(fromId, quiz.question, shuffled.options, {
        correct_option_id: shuffled.correctIndex,
        is_anonymous: false,
        explanation: quiz.explanation
      });
  ...
  bot.action('take_prophetic_quiz', async (ctx) => {
    ...
    try {
      // 🎲 Shuffle options so correct answer position varies randomly
      const shuffled = shuffleQuizOptions(sit.quiz.options, sit.quiz.correct_option_index || 0);
      await bot.telegram.sendQuiz(fromId, sit.quiz.question, shuffled.options, {
        correct_option_id: shuffled.correctIndex,
        is_anonymous: false,
        explanation: sit.quiz.explanation
      });
