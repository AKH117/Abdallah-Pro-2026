...
export function registerHandlers(bot) {
  ...
  function isMedicalErrorExplanation(raw) {
    ...
    // 1. Structural indicators from the 5-step framework
    const hasStepEmojis = /[1-5]️⃣|[1-5]-|أولاً|ثانياً|ثالثاً|رابعاً|خامساً/i.test(raw);
    const hasQuestionMarker = /❓|السؤال\s*(?:الأول|الثاني|الثالث|الرابع|الخامس|\d+)?|question\s*\d*|case vignette/i.test(raw);
    const hasMnemonicMarker = /تحشيشة|نيمونيك|التحشيشة الماسية|نصائح ذهبية|golden tip/i.test(raw);
    const hasCoreKeywords = /ترجمة رأس السؤال|المصطلحات|الإجابة الصح|السبب التشريحي|السبب الفسيولوجي|المشتتات|باقي الإجابات|تذكير بالمنهج|حدود رسمة القلب/i.test(raw);

    if ((hasStepEmojis && hasCoreKeywords) || (hasQuestionMarker && hasCoreKeywords) || (hasMnemonicMarker && hasCoreKeywords)) return true;
    if (hasMnemonicMarker && hasStepEmojis) return true;
    if (hasQuestionMarker && hasStepEmojis) return true;

    // 2. Fragment / split chunk indicators (e.g. part 2 of a split message)
    if (/^(?:الحد الأيسر|الحد الأيمن|5️⃣|4️⃣|3️⃣|2️⃣|إجابة\s+.*غلط|التحشيشة الماسية)/i.test(raw)) return true;

    // 3. Fallback hit counter
    let hits = 0;
    if (t.includes('ترجمة') || t.includes('المصطلحات')) hits++;
    if (t.includes('الإجابة الصح') || t.includes('السبب التشريحي') || t.includes('السبب الفسيولوجي')) hits++;
    if (t.includes('المشتتات') || t.includes('باقي الإجابات') || t.includes('خيارات غلط') || (t.includes('إجابة') && t.includes('غلط'))) hits++;
