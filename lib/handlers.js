...
export function registerHandlers(bot) {
  ...
  bot.use(async (ctx, next) => {
    ...
    if (ctx.chat && ctx.chat.type === 'private') {
      ctx.setChatMenuButton({
        type: 'web_app',
        text: '📱 لوحة التحكم',
        web_app: { url: `https://abdallah-pro.onrender.com/?telegram_id=${fromId}` }
      }).catch(() => {});
    }
  ...
  if (ADMIN_CHAT_ID) {
    bot.telegram.callApi('setChatMenuButton', {
      ...
      menu_button: JSON.stringify({
        type: 'web_app',
        text: '📱 لوحة التحكم',
        web_app: { url: `https://abdallah-pro.onrender.com/?telegram_id=${ADMIN_CHAT_ID}` }
      })
    }).catch(() => {});
  }
  ...
  bot.command(['rename', 'name', 'اسم'], async (ctx) => {
    ...
    return ctx.reply(successMsg, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🚀 فتح القائمة الرئيسية', callback_data: 'menu_main' }],
          [{ text: '📱 لوحة التحكم', web_app: { url: `https://abdallah-pro.onrender.com/?telegram_id=${fromId}` } }]
        ]
      }
    });
  });
  ...
  async function sendStudentStats(ctx, fromId) {
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '📱 فتح لوحة التحكم الكاملة', web_app: { url: `https://abdallah-pro.onrender.com/?telegram_id=${fromId}` } }],
        [
