...
// Ultra-Fast Supported Gemini Flash Models Hierarchy (Ranked strictly by lowest latency ~1.8s)
export const GEMINI_MODELS = [
  'gemini-3.6-flash',      // Primary Active Official Model
  'gemini-3.5-flash-lite', // High-Speed Backup
...
export function getBalancedSlots(keys, models = GEMINI_MODELS) {
  const normKeys = Array.isArray(keys) ? keys : extractApiKeys(keys);
  if (!normKeys || normKeys.length === 0) return [];

  const now = Date.now();
  // Filter out temporarily invalid keys (cooldown: 5 minutes)
  const activeKeys = normKeys.filter(k => {
  ...
  const targetKeys = activeKeys.length > 0 ? activeKeys : normKeys;
...
// Fast & Resilient HTTP Request to Gemini REST API via Native Fetch
export async function sendGeminiRequest(modelName, apiKey, postData) {
  try {
    ...
    const textPart = candidate?.content?.parts?.find(p => p.text && !p.thought) || candidate?.content?.parts?.[0];
    if (res.ok && textPart?.text) {
      return { ok: true, data: parsed, text: textPart.text, status: res.status };
    }

    const errObj = parsed.error || {};
    const isQuota = res.status === 429 || errObj.code === 429 || errObj.status === 'RESOURCE_EXHAUSTED' || (errObj.message && errObj.message.includes('quota'));
    const isForbidden = res.status === 401 || res.status === 403 || errObj.code === 401 || errObj.code === 403;
    if (isForbidden && apiKey) {
      _invalidKeyTimestamps.set(apiKey, Date.now());
    }
    return { ok: false, isQuota, isForbidden, status: res.status, error: errObj };
  } catch (err) {
    ...
    return {
      ...
      error: { message: isTimeout ? 'Gemini request timeout' : err.message }
    };
  }
}
...
🏛️ المجموعة الأكاديمية المعتمدة لـ د. عبدالله: جروب 7 (Group 7 - Block 7 - المجموعات الفردية)
📍 جدول وسكاشن وقاعات جروب 7 المعتمدة رسمياً (توقيت القاهرة الحالي: ${cairoTimeStr} - ${dayName}):
- الأحد:
  * 08:45 - 10:25 ص: 🫀 محاضرة مدرج CAD Intro to Cardiology
  * 10:30 - 12:10 م: 🫀 محاضرة مدرج CAD Normal ECG Blueprint
  * 12:30 - 02:10 م: 💬 سكشن SGD CAD (مناقشة حالات صمامات وقلب) — بالمستشفى (قاعة 14)
  * 02:15 - 03:55 م: 🩺 راوند SGT CAD إكلينيكي — المستشفى (الأقسام الداخلية والعيادات)
- الإثنين:
  * يوم المهارات والاستذكار الحر لموديول القلب: Open Lab في المستشفى (قاعة 1)
- الثلاثاء:
  * 08:45 - 10:25 ص: 🫀 محاضرة مدرج CAD Mitral Valve Diseases
  * 10:30 - 12:10 م: 👶 محاضرة مدرج PED Growth & Assessment
  * 12:30 - 02:10 م: 🏥 سكشن Alex CAD — بالمستشفى (قاعة 15)
  * 02:15 - 03:55 م: 🩺 راوند Clinical CAD كلينيكال باطنة وقلب — بالمستشفى (قاعة 10)
- الأربعاء:
  * 08:45 - 10:25 ص: 👶 محاضرة مدرج PED Disorders of Physical Growth
  * 10:30 - 12:10 م: 🫀 محاضرة مدرج CAD Aortic Valve Diseases
  * 12:30 - 02:10 م: 🔬 سكشن Skill Lab 7 معمل المهارات — بالمستشفى (قاعة 4)
  * 02:15 - 03:55 م: 📋 راوند Cases 7&8 مناقشة حالات سريرية — مبنى 3 (قاعة 3103)
- الخميس:
  * 08:45 - 10:25 ص: 🫀 محاضرة مدرج CAD Tricuspid & Prosthetic Valves
  * 10:30 - 12:10 م: 🫀 محاضرة مدرج CAD Atherosclerosis & Lipids
  * 12:30 - 02:10 م: 👶 سكشن SGT & SGD PED أطفال — مبنى 2 (قاعة 2018)
  * 02:15 - 03:55 م: 👶 راوند Clinical PED أطفال سريري — بالمستشفى (قاعة 12)

⚠️ قاعدة استجابة حاسمة لسكاشن د. عبدالله (جروب 7):
إذا سألك د. عبدالله فويس أو كتابة عن: "عندي ايه دلوقتي؟", "سكشني فين؟", "قاعتي ايه؟", "جدول النهاردة", "السكاشن اللي عليا":
➔ أجب فوراً بدقة متناهية من الجدول المعتمد أعلاه حسب اليوم والوقت الحالي بالقاهرة (${dayName} ${cairoTimeStr})!
➔ حدد له بالاسم: اسم المحاضرة أو السكشن، التوقيت، ورقم القاعة والمبنى (مثل قاعة 14 بالمستشفى، أو قاعة 3103 بمبنى 3، إلخ)، وشجعه بأسلوب طبي ودود!
➔ إياك أن تقول له "أرسل لي الجدول"؛ لأن جدوله لجروب 7 محفوظ ومبرمج عندك بالفعل!
...
1. 🌟 أنواع النوايا المدعومة (detected_type):
   ...
   - "record_mnemonic": تسجيل تحشيشة أو نيمونيك طبية أو إسكيمة أو Pearl (مثال: "سجل تحشيشة لتضخم الأذين", "نيمونيك جديدة في الكارديو", "تحشيشة: الأذين الأيسر كئيب بـ 2").
...
2. 📋 قواعد الاستخراج:
    - ⏰ المواعيد والتذكيرات (حاسم وإلزامي جداً):
      * أي طلب لتذكير الطالب بأي أمر بعد مدة معينة أو في وقت محدد (مثال: "فكرني بعد دقيقة اشرب ماء", "فكرني بعد نص ساعة اراجع مخالفات والدي", "فكرني الساعة 9 بالليل اذاكر", "عايزك تجدولي معاد بعد المغرب"):
        ➔ يجب إلزامياً وبلا استثناء استخراجه داخل data.appointments وممنوع وضعه في tasks فقط!
        ➔ احسب التاريخ والوقت بدقة ISO بالزمن المصري (+03:00) وضعه في due_datetime (مثال: "2026-09-10T21:11:00+03:00").
        ➔ ضع title واضح ومباشر للأمر المطلوب تذكيره به (مثل: "شرب الماء", "مراجعة مخالفات الوالد على الموقع").
        ➔ ضع notes توضح التفاصيل أو المدة المطلوبة.
    ...
    - 📚 المذاكرة وموديولات الطب (Study Sessions & Medical Tracking):
      * استخرج في data.study:
        - course_code (مثال: CAD402 أو CAD لموديول القلب، PED401 أو PED لطب الأطفال، إلخ).
        - topic: الموضوع الطبي أو الشابتر (مثل "مقدمة الكارديولوجي ورسم القلب Normal ECG" أو "صمامات القلب").
        - duration_minutes: حول الساعات دائماً إلى دقائق في duration_minutes (ساعتين ➔ 120، ساعة ➔ 60، إذا لم يحدد وقتاً صراحة ضع 60 كافتراضي).
        - from_page: رقم أول صفحة (كرقم صحيح، مثلاً 1).
        - to_page: رقم آخر صفحة (كرقم صحيح، مثلاً 9).
        - pages_covered: إجمالي عدد الصفحات المذاكرة.
        - comprehension_rating: درجة الاستيعاب من 1 إلى 5 (افتراضياً 5).
      * ⚠️ قاعدة المجالات والصفحات المتعددة (Multiple Page Ranges):
        إذا ذكر الطالب أكثر من جزء أو نطاق صفحات (مثل: "ذاكرت من صفحة 1 لـ 9 ومن 37 لـ 40 موديول القلب"):
        ➔ سجل كل نطاق صفحات كعنصر مستقل في مصفوفة data.study (الأول: from_page: 1, to_page: 9, pages_covered: 9 / الثاني: from_page: 37, to_page: 40, pages_covered: 4).
        ➔ هذا يمكن خوارزمية التكرار المتباعد (SRS) من جدولة وتتبع ومراجعة كل جزء بشكل مستقل وبدقة.
      * في conversational_reply:
        - رحب بإنجازه وشجعه بأسلوب طبي أخوي راقٍ ودافئ.
        - اذكر له الصفحات المنجزة والموديول بوضوح.
        - قارن تقدمه بجدول الكلية لجروب 7 المعتمد في المنظومة، ووضّح له موقعه الحالي (مثال: "ما شاء الله يا دكتور، إنجاز ممتاز ومذاكرتك للصفحات دي ماشية على نفس وتيرة محاضرات الأحد والإثنين (CAD & Normal ECG)، والخطوة التالية لمواكبة الكلية هي...").
        - أكد له أن هذه الصفحات تم تسجيلها وجدولتها بنجاح في منحنى التثبيت والمراجعة المتباعدة (SRS) لتذكيره واختباره فيها تلقائياً.
   ...
   - 🗣️ الإنجليزية ونصوص المحادثات (English AI Chat & Flashcards Engine):
    ...
    - 🩺📖 شروحات ومذكرات المذاكرة الخاصة بالطالب (User Study Explanations & High-Yield Notes):
      - إذا أرسل الطالب جزء شرح دراسي، أو تلخيصاً لمفهوم طبي، أو ملاحظة سريرية/نصاً يريد حفظه وتثبيته في عقله بالتكرار المتباعد (سواء نص من كتاب، أو تفريغ محاضرة، أو تريكة راوند):
        ➔ استخرج تفاصيل الشرح في data.study_explanations كمصفوفة كائنات:
           [
             {
               "course_code": "CAD402" أو كود الموديول المناسب حسب السياق,
               "topic": "عنوان المفهوم أو الشرح",
               "explanation_text": "نص الشرح والتفاصيل بالكامل",
               "key_takeaway": "الخلاصة المركزة للحفظ والتثبيت",
               "notes": "أي ملاحظات إضافية"
             }
           ]
        ➔ واستخرج أيضاً جلسة تحصيل في data.study لتوثيق وقت المذاكرة وإحصائيات الإنجاز.
    - ❓🩺 أسئلة وكويزات الطالب للتثبيت (User-Submitted Medical Quizzes & MCQs):
      - إذا أرسل الطالب سؤالاً، كويزاً، مسألة سريرية، أو سؤال MCQ للمراجعة والتثبيت:
        ➔ استخرج السؤال في data.medical_quizzes كمصفوفة كائنات:
           [
             {
               "course_code": "CAD402" أو كود الموديول المناسب,
               "topic": "موضوع السؤال",
               "question": "نص السؤال الدقيق",
               "options": ["خيار 1", "خيار 2", "خيار 3", "خيار 4"],
               "correct_option_index": 0,
               "answer_and_explanation": "الإجابة النموذجية وتفسيرها وتريكة الراوند",
               "doctor_pearl": "تريكة سريرية مركزة",
               "is_user_submitted": true
             }
           ]
     - 💎🩺 التحشيشات والـ Medical Pearls والنيمونيك (Mnemonics & Pearls Engine):
       - إذا قال الطالب (فويس أو نص): "سجل تحشيشة...", "نيمونيك جديدة...", "تحشيشة لكذا...", "إسكيمة...", أو أملى تحشيشة وربطاً ذهنياً لحفظ معلومة طبية:
         ➔ اضبط "detected_type": "record_mnemonic"
         ➔ املأ data.medical_mnemonic = {
              "course_code": "CAD402" أو كود الموديول التابع له,
              "topic": "الموضوع الطبي الدقيق للتحشيشة",
              "mnemonic_text": "نص التحشيشة أو الربط أو الإسكيمة بالكامل",
              "explanation_pearl": "المعنى الطبي والسريري وتطبيقها في الامتحان أو الراوند"
            }
${dynamicSections}
${prefsRestrictions}
...
هيكل الـ JSON المطلوب بدقة:
{
  ...
  "data": {
    ...
    "study": [
      {
        "course_code": "CAD402",
        "topic": "مقدمة الكارديولوجي ورسم القلب Normal ECG",
        "duration_minutes": 60,
        "from_page": 1,
        "to_page": 9,
        "pages_covered": 9,
    ...
    "appointments": [
      {
        "title": "مراجعة مخالفات الوالد على الموقع",
        "due_datetime": "2026-09-10T21:11:00+03:00",
        "remind_at": "2026-09-10T21:11:00+03:00",
        "notes": "تذكير مجدول حسب طلب الطالب"
      }
    ],
    "thoughts": [],
    "self_development": [],
...
The student (${studentName}) recorded an English/Arabic speech, monologue, or live conversational transcript.

CRITICAL MANDATE — EXHAUSTIVE EXTRACTION (ZERO SKIPPING / ZERO SUMMARIZATION):
1. Spoken speech-to-text transcripts often run on without punctuation. You MUST mentally segment the text into EVERY SINGLE individual sentence, clause, thought, and narrative step from the VERY FIRST WORD to the VERY LAST WORD.
2. DO NOT combine, condense, or summarize multiple thoughts into one! Every distinct statement, explanation, example, or feeling MUST be its own separate flashcard.
3. DO NOT stop after 10 or 15 items! If the speech contains 25, 30, 40, or 50 sentences, you MUST return 25, 30, 40, or 50 cards!
4. STRICT SEQUENTIAL COVERAGE: You must process the speech sequentially until the VERY END. The final cards in the array MUST cover the ending thoughts of the student (e.g. searching for income/money, talking to father, talking to uncle, talking to cousin, trying out ideas, etc.). If you stop halfway or truncate, the output is considered unacceptable.
5. Input handling:
   - If bilingual (marked with "AR" and "EN"): Extract every single sentence/clause from the text into a polished English sentence matched with its authentic Egyptian Arabic meaning.
   - If English only (or "EN"): Polish each spoken sentence into clean, natural native conversational English (fixing speech-to-text glitches or slips while keeping 100% of the authentic meaning), paired with Egyptian colloquial translation.
   - If Arabic only (or "AR"): Translate and formulate every single Arabic thought/sentence into natural, fluent, native spoken English sentences with smooth conversational transitions, paired with the Arabic original.
6. Egyptian Arabic & English Alignment Rules:
   - Provide the EXACT, authentic Egyptian colloquial translation as spoken, NEVER modify or 'correct' the student's spoken Egyptian words!
   - Make the English translation sound like a natural, native English speaker talking to a friend. AVOID stiff, robotic, or overly legalistic vocabulary (e.g. use "He says one thing and we say another", NOT "He claims...").
   - Faithfulness to idiom meaning: translate "كل الأفكار دي وبرضه مش عارف..." to "Even with all these ideas, I still can't figure out...", NEVER "No matter what ideas come up".
   - Accuracy in family and business terms: "خالي" is "my uncle", "ابن خالتي" is "my cousin". Never swap them.
   - DO NOT invent extra prefix clauses that were never spoken (e.g. if the speaker said "زي صور العقود وصور الإذون", translate "Like photos of contracts and permission vouchers...", DO NOT invent "We lost important files like...").
7. Also extract any key idioms, phrasal verbs, or technical terms as separate bonus cards if applicable.

Output strictly valid JSON with this exact schema:
{
  "detected_type": "life_actions",
  "conversational_reply": "تشجيع دافئ وحماسي جداً لدكتور عبدالله بالعامية المصرية مع تأكيد توثيق كل جملة وفكرة قالها بدقة في بنك التكرار المتباعد للطلاقة",
  "summary_text": "ملخص توثيق جلسة الإنجليزية والطلاقة التامة",
  "data": {
    "english_flashcards": [
      {
        "term_or_sentence": "Full natural English sentence or idiom/phrase",
        "egyptian_translation": "الترجمة الدقيقة بالعامية المصرية الدارجة",
...
// ==============================================================================
// 🩺 31 & 32. استيعاب شروحات أخطاء الأسئلة الطبية وبنك التحشيشات والـ Pearls
// ==============================================================================

export async function parseMedicalErrorExplanation(text, keyInput, studentName = 'د. عبدالله') {
  const keys = extractApiKeys(keyInput);
  if (keys.length === 0) throw new Error('مفاتيح Gemini غير متوفرة');

  const prompt = `You are Dr. Abdullah's Medical Education Specialist and Spaced Repetition Engine.
The student (${studentName}) sent a text containing one OR MORE detailed medical questions he previously answered incorrectly, explained using a 5-step structured methodology:
1. ترجمة رأس السؤال والمصطلحات الصعبة (Question translation & difficult medical jargon)
2. ليه الإجابة الصح صح (Correct answer mechanism - anatomical, physiological, radiological, pathological)
3. ليه باقي الإجابات التانية غلط (Why each incorrect distractor is wrong)
4. تذكير بالمنهج والمنطقة المحيطة بالسؤال (High-yield curriculum summary & surrounding anatomy/leads/pathways)
5. نصائح ذهبية وتحشيشة للحفظ صم (Golden exam tips and Mnemonic scheme)

CRITICAL INSTRUCTIONS:
- The text may contain MULTIPLE questions (e.g. Question 1, Question 2, etc.), or parts of questions split across messages.
- Extract ALL questions present in the text into the "questions" array.
- If there is 1 question, return an array with 1 item. If there are 2 or more, return ALL of them in order.
- Determine the appropriate 4th-year medical course code: 'CAD402' (Cardiology), 'PED401' (Pediatrics), 'RSD403' (Respiratory), 'HVD404' (Hematology), 'SKL 7' (Skills), or 'GEN'.
- In "difficult_terms": Extract EVERY term specifically listed under "المصطلحات اللي ممكن تعمل أزمة" or "المصطلحات الصعبة" (e.g. ST-elevation, Leads II, III, aVF, Occluded), with its exact English term in "term" and its detailed Arabic explanation in "meaning".
- In "mnemonic_text": Extract ONLY the core catchy mnemonic rhyme, pun, or short punchline phrase (e.g. "الأذين الأيسر لما بيكبر بيبقى كئيب بـ 2"). STRICTLY DO NOT append explanations, colon phrases like ": يعني بيعمل...", or medical reasoning inside "mnemonic_text". Put all explanations in "golden_tip" or "correct_mechanism".
...
Output strictly valid JSON with this exact schema:
{
  "questions": [
    {
      "course_code": "CAD402",
      "topic": "Concise medical topic title in Arabic or English",
      "question_en": "The exact English clinical scenario or question stem",
      "question_ar": "ترجمة السؤال باللغة العربية",
      "difficult_terms": [
        { "term": "Term in English", "meaning": "شرح المصطلح ودلالته السريرية بالعربي" }
      ],
      "correct_answer": "Exact correct option title",
      "correct_mechanism": "الشرح العلمي/التشريحي/الفسيولوجي لسبب صحة الإجابة بالتفصيل",
      "distractors": [
        { "option": "اسم الخيار الخاطئ", "reason": "لماذا هذا الخيار خاطئ ولمن ينتمي سريرياً" }
      ],
      "curriculum_context": "الملخص المركز للمنطقة المحيطة بالسؤال في المنهج",
      "golden_tip": "النصيحة الذهبية لحل هذا النوع من الأسئلة",
      "mnemonic_text": "التحشيشة أو الإسكيمة اللفظية المركزة فقط (بدون شرح أو يعني بيعمل...)"
    }
  ],
  "conversational_reply": "تشجيع طبي أخوي دافئ لدكتور عبدالله بالعامية المصرية مع تلخيص سريع للأسئلة والتحشيشات المستخرجة"
}`;
  ...
  const sanitizeMnemonicFields = (item) => {
    ...
    if (item.mnemonic_text && typeof item.mnemonic_text === 'string') {
      ...
      if (m) {
        ...
        if (!item.golden_tip && m[2]) {
          item.golden_tip = m[2].trim();
        }
      }
    }
  };
  ...
  for (const { model, key } of slots) {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(parsed.questions) && parsed.questions.length > 0) {
        parsed.questions.forEach(sanitizeMnemonicFields);
        return parsed;
      }
      if (parsed.question_en || parsed.topic || parsed.correct_answer) {
        sanitizeMnemonicFields(parsed);
        return {
          questions: [parsed],
          conversational_reply: parsed.conversational_reply || 'تم استيعاب السؤال الطبي بنجاح! 🎯'
        };
      }
      return parsed;
    }
  ...
  throw new Error('تعذر معالجة وتفكيك شرح السؤال الطبي مؤقتاً.');
}

export async function parseMedicalMnemonic(input, keyInput, isAudio = false, studentName = 'د. عبدالله') {
  const keys = extractApiKeys(keyInput);
  if (keys.length === 0) throw new Error('مفاتيح Gemini غير متوفرة');

  const prompt = `You are Dr. Abdullah's Medical Mnemonic & Pearls Specialist.
The student (${studentName}) recorded a voice note or sent a message dictating a medical mnemonic (تحشيشة / إسكيمة / نيمونيك) or clinical pearl.
He might say phrases like:
- "سجل تحشيشة لتضخم الأذين..."
- "نيمونيك جديدة في الكارديو..."
- "تحشيشة الصمام الميترالي كذا كذا..."
- or directly dictate a mnemonic scheme in Egyptian colloquial / medical English.

YOUR TASK:
Transcribe/understand the mnemonic, identify the relevant course code ('CAD402' for cardiac, 'PED401' for pediatric, 'RSD403' for chest, 'HVD404' for hematology, 'SKL 7' for emergency/skills), extract the hook and the clinical pearl.

Output strictly valid JSON with this exact schema:
{
  "course_code": "CAD402",
  "topic": "الموضوع الطبي الدقيق للتحشيشة (مثل: تضخم الأذين الأيسر في أشعة الصدر)",
  "mnemonic_text": "نص التحشيشة أو الإسكيمة أو بيت الشعر أو الربط الذهني",
  "explanation_pearl": "المعنى الطبي والسريري للتحشيشة وتطبيقه في الامتحان أو الراوند",
  "conversational_reply": "رد دافئ ومشجع لدكتور عبدالله بالعامية المصرية يؤكد حفظ التحشيشة في بنك النيمونيك والـ Pearls"
}`;
  ...
  if (isAudio) {
    ...
    parts.push({ text: `نص الرسالة أو التحشيشة:\n"${input}"` });
  }
  ...
  for (const { model, key } of slots) {
    const res = await sendGeminiRequest(model, key, postData);
    if (res.ok) {
      const parsedRes = extractAndParseJson(res.text);
      if (parsedRes && parsedRes.mnemonic_text) {
        const m = parsedRes.mnemonic_text.match(/^(.*?)(?::\s*(?:يعني|وده|علشان|عشان|أي)\s*([\s\S]*))$/);
        if (m) {
          parsedRes.mnemonic_text = m[1].trim();
          if (!parsedRes.explanation_pearl && m[2]) {
            parsedRes.explanation_pearl = m[2].trim();
          }
        }
      }
      return parsedRes;
  ...
  throw new Error('تعذر معالجة وتسجيل التحشيشة الطبية مؤقتاً.');
}
