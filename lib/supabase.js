...
export async function processStudentPollAnswer(pollId, selectedOptionIndex, telegramId) {
  ...
  try {
    ...
    if (isCorrect) {
      ...
      if (isNonMed) {
        // Non-medical weekly spaced intervals: 7d, 14d, 28d, 60d, 90d, 180d -> Mastered at Level 6!
        const nonMedIntervals = { 1: 168, 2: 336, 3: 672, 4: 1440, 5: 2160, 6: 4320 };
        nextHours = nonMedIntervals[nextLevel] || 168;
      } else {
        // Medical intervals: 1d (24h), 3d (72h), 7d (168h), 14d (336h), 30d (720h), 60d (1440h) -> Mastered at Level 6!
    ...
    await supabase.from('medical_spaced_quizzes').update({
      ...
      is_mastered: isCorrect && nextLevel >= 6,
