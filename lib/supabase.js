...
export async function saveNativeQuizPoll(telegramId, quizData) {
  ...
  // 🛡️ Guard: Do not store or schedule Sharia, Fiqh, Statesman, or Past Ruler mistake quizzes
  if (quizData.course_code === 'SHARIA' || quizData.course_code === 'STATESMAN' || /فقه|شريع|دولة|مضوا/.test(quizData.topic || '') || /فقه|شريع|دولة|مضوا/.test(quizData.question || '')) {
...
export async function scheduleAdminSpacedQuiz(telegramId, { course_code = 'SHARIA', topic, question, options, correct_option_index = 0, explanation = '', delayHours = 2.0 }) {
  // 🛡️ Guard: Do not schedule Sharia, Fiqh, Statesman, or Past Ruler mistake quizzes
  if (course_code === 'SHARIA' || course_code === 'STATESMAN' || /فقه|شريع|دولة|مضوا/.test(topic || '') || /فقه|شريع|دولة|مضوا/.test(question || '')) {
    return { success: false, skipped: true, reason: 'Disabled by user preference' };
  }
  ...
  // 🎲 Shuffle options so correct answer is randomly distributed (never locked to index 0)
  const shuffled = shuffleQuizOptions(options || [], correct_option_index || 0);
  const finalOptions = shuffled.options;
  const finalCorrectIndex = shuffled.correctIndex;
