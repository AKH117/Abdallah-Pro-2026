...
export async function processStudentPollAnswer(pollId, selectedOptionIndex, telegramId) {
  ...
  try {
    ...
    // Spaced repetition: Correct answers MUST NOT repeat except after long periods (Level 1: 5 days, Level 2: 12 days, ...)
    // Incorrect answers repeat after 24h for concept consolidation.
    let nextLevel;
    let nextHours;
    if (isCorrect) {
      nextLevel = Math.min(6, (quiz.repetition_level || 0) + 1);
      const intervals = { 1: 120, 2: 288, 3: 600, 4: 1200, 5: 2160, 6: 4320 }; // 5d, 12d, 25d, 50d, 90d, 180d
      nextHours = intervals[nextLevel] || 120;
    } else {
      nextLevel = 0;
      nextHours = 24; // 24 hours to review wrong answer tomorrow
    }
...
export async function scheduleAdminSpacedQuiz(telegramId, { course_code = 'SHARIA', topic, question, options, correct_option_index = 0, explanation = '', delayHours = 2.0 }) {
  // 🛡️ Permanent Guard: All non-medical admin curricula quizzes are completely disabled by user preference
  return { success: false, skipped: true, reason: 'Non-medical quizzes are permanently disabled by user preference' };
}
