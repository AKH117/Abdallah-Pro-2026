...
export async function getUserProfile(userId) {
  ...
  if (numId === ADMIN_CHAT_ID) {
    ...
    return {
      ...
      custom_courses: storedSession?.profile?.custom_courses || [
        { code: 'CAD402', title: 'أمراض وجراحة القلب والأوعية (Cardiology)' },
        { code: 'RSD403', title: 'أمراض وجراحة الصدر والجهاز التنفسي (Chest)' },
        { code: 'HVD404', title: 'أمراض الدم وجراحة الأوعية الدموية (Hematology & Vascular)' },
        { code: 'PED401', title: 'طب الأطفال 1 (Pediatrics 1 - ممتد)' },
        { code: 'SKL 7', title: 'المهارات الإكلينيكية والـ OSCE (Clinical Skills 7)' },
        { code: 'PRF 7', title: 'السلوك والاحترافية المهنية (Professionalism)' },
        { code: 'ELE 7', title: 'المقرر الاختياري الإلكتروني (Elective)' }
      ],
  ...
  try {
    ...
    if (userRow || (storedSession?.profile && (storedSession.profile.full_name || storedSession.profile.trial_ends_at || storedSession.profile.created_at))) {
      const now = Date.now();
      const sessProf = storedSession?.profile || {};

      const userTrialEnd = userRow?.trial_ends_at ? new Date(userRow.trial_ends_at).getTime() : 0;
      const sessTrialEnd = sessProf.trial_ends_at ? new Date(sessProf.trial_ends_at).getTime() : (sessProf.created_at ? new Date(sessProf.created_at).getTime() + 3 * 24 * 3600 * 1000 : 0);
      const effectiveTrialEnd = Math.max(userTrialEnd, sessTrialEnd);

      const userSubEnd = userRow?.subscription_ends_at ? new Date(userRow.subscription_ends_at).getTime() : 0;
      const sessSubEnd = sessProf.subscription_ends_at ? new Date(sessProf.subscription_ends_at).getTime() : 0;
      const effectiveSubEnd = Math.max(userSubEnd, sessSubEnd);

      let status = 'trial';
      if (userRow?.subscription_status === 'lifetime' || sessProf.subscription_status === 'lifetime') {
        status = 'lifetime';
      } else if ((userRow?.subscription_status === 'active' || sessProf.subscription_status === 'active') && effectiveSubEnd > now) {
        status = 'active';
      } else if (effectiveSubEnd > now) {
        status = 'active';
      } else if (effectiveTrialEnd > now) {
        status = 'trial';
      } else {
        status = 'expired';
      }
      ...
      if (status === 'lifetime') {
        isActive = true;
      } else if (status === 'active' && effectiveSubEnd > now) {
        isActive = true;
      } else if (status === 'trial' && effectiveTrialEnd > now) {
        isActive = true;
      } else {
        isActive = false;
        status = 'expired';
      ...
      const mergedPrefs = {
        ...DEFAULT_USER_PREFERENCES,
        ...(sessProf.preferences || {}),
        ...(userRow?.preferences || {})
      };

      const resolvedName = userRow?.full_name || sessProf.full_name || 'طالب زميل';
      const resolvedGender = userRow?.gender || sessProf.gender || detectGenderFromName(resolvedName);
      const targetEnd = status === 'active' ? effectiveSubEnd : (status === 'trial' ? effectiveTrialEnd : 0);
      const daysRemaining = status === 'lifetime' ? 3650 : (isActive ? Math.max(1, Math.ceil((targetEnd - now) / (24 * 3600 * 1000))) : 0);
      ...
      const profResult = {
        ...
        username: userRow?.username || sessProf.username || null,
        university: userRow?.university || sessProf.university || 'كلية الطب البشري',
        academic_year: userRow?.academic_year || sessProf.academic_year || 'الفرقة الرابعة',
        semester: userRow?.semester || sessProf.semester || 'الترم الأول',
        custom_courses: (userRow?.custom_courses && userRow.custom_courses.length > 0) ? userRow.custom_courses : (sessProf.custom_courses || []),
        preferences: mergedPrefs,
        role: (userRow?.role === 'admin' || sessProf.role === 'admin') ? 'admin' : 'student',
        subscription_status: status,
        is_active: isActive,
        is_admin: userRow?.role === 'admin' || sessProf.role === 'admin',
        is_trial: status === 'trial' && isActive,
        trial_ends_at: effectiveTrialEnd ? new Date(effectiveTrialEnd).toISOString() : null,
        subscription_ends_at: effectiveSubEnd ? new Date(effectiveSubEnd).toISOString() : null,
...
export async function updateUserAcademicProfile(userId, { academicYear, semester, university, customCourses, academicGroup }) {
  if (!userId) return null;
  const numId = Number(userId);
  invalidateUserProfileCache(numId);
  const currentProf = await getUserProfile(userId);
  ...
  const updates = {
    ...
    academic_group: academicGroup !== undefined ? academicGroup : (currentProf?.academic_group || 'جروب 7')
  };
...
export async function updateUserAcademicGroup(userId, groupStr) {
  if (!userId || !groupStr) return null;
  const numId = Number(userId);
  invalidateUserProfileCache(numId);
  try {
    await supabase.from('users').update({
      academic_group: groupStr,
      updated_at: new Date().toISOString()
    }).eq('telegram_id', numId);
  } catch (_) {}
  ...
  const sessData = existing?.data || {};
  if (!sessData.profile) sessData.profile = { telegram_id: numId };
  sessData.profile.academic_group = groupStr;
...
export async function activateUserSubscription(userId, days = 120, adminNotes = 'تفعيل اشتراك فصلي') {
  ...
  try {
    ...
    // 2. Update users table without wiping any user data (excluding gender column)
    try {
      ...
      console.warn('[activateUserSubscription users table warn]:', dbErr.message);
    }
...
// Helper: User Session Storage in Supabase with In-Memory Cache
const userSessionCache = new Map();
const USER_SESSION_CACHE_TTL = 45000; // 45 seconds fast cache

export function invalidateUserSessionCache(userId) {
  if (userId) userSessionCache.delete(Number(userId));
}
...
export async function setUserSession(userId, sessionData) {
  try {
    if (!userId) return;
    const numId = Number(userId);
    userSessionCache.delete(numId);
    const { data: existing } = await supabase.from('bot_sessions').select('*').eq('chat_id', numId).maybeSingle();
    const merged = { ...(existing?.data || {}), ...sessionData };
    userSessionCache.set(numId, { data: merged, timestamp: Date.now() });
    await supabase.from('bot_sessions').upsert({
      chat_id: numId,
...
export async function getUserSession(userId) {
  try {
    if (!userId) return null;
    const numId = Number(userId);
    const cached = userSessionCache.get(numId);
    if (cached && (Date.now() - cached.timestamp < USER_SESSION_CACHE_TTL)) {
      return cached.data;
    }
    const { data } = await supabase
      .from('bot_sessions')
      ...
      .eq('chat_id', numId)
      .maybeSingle();
    const sessData = data?.data || null;
    userSessionCache.set(numId, { data: sessData, timestamp: Date.now() });
    return sessData;
  } catch (e) {
    return null;
  }
...
export async function saveNativeQuizPoll(telegramId, quizData) {
  ...
  // 🛡️ Guard: Do not store or schedule Sharia, Fiqh, Statesman, Discipline, Sahaba, Bukhari, or Quran Asbab quizzes
  const nonMed = ['SHARIA', 'STATESMAN', 'DISCIPLINE', 'SAHABA_SPOTLIGHT', 'BUKHARI', 'QURAN_ASBAB', 'PROPHETIC_LEADERSHIP'];
  if (nonMed.includes(quizData.course_code) || /فقه|شريع|دولة|مضوا|أعصاب|دوبامين|صحابي|بخاري|نزول/.test(quizData.topic || '') || /فقه|شريع|دولة|مضوا|أعصاب|دوبامين|صحابي|بخاري|نزول/.test(quizData.question || '')) {
    return { poll_id: pollId, skipped: true };
  }

  const metaObj = {
    poll_id: pollId,
    message_id: quizData.message_id || quizData.telegram_message_id || null,
    chat_id: quizData.chat_id || numId,
  ...
  try {
    ...
    if (existing && existing.length > 0) {
      const ex = existing[0];
      let exMeta = {};
      if (ex.doctor_pearl && ex.doctor_pearl.includes('<<<QUIZ_META_START>>>')) {
        try {
          const json = ex.doctor_pearl.split('<<<QUIZ_META_START>>>')[1].split('<<<QUIZ_META_END>>>')[0];
          exMeta = JSON.parse(json);
        } catch (_) {}
      }
      const mergedMeta = { ...exMeta, ...metaObj };
      // Preserve active poll_id, message_id, chat_id, shuffled options, and correct index if not provided in update
      if (!metaObj.poll_id && exMeta.poll_id) {
        mergedMeta.poll_id = exMeta.poll_id;
        mergedMeta.options = exMeta.options;
        mergedMeta.correct_index = exMeta.correct_index;
      }
      if (!metaObj.message_id && exMeta.message_id) {
        mergedMeta.message_id = exMeta.message_id;
      }
      if (!metaObj.chat_id && exMeta.chat_id) {
        mergedMeta.chat_id = exMeta.chat_id;
      }

      await supabase.from('medical_spaced_quizzes').update({
        answer_and_explanation: quizData.explanation || ex.answer_and_explanation,
        doctor_pearl: `<<<QUIZ_META_START>>>${JSON.stringify(mergedMeta)}<<<QUIZ_META_END>>> ${quizData.explanation || ex.doctor_pearl || ''}`.trim()
      }).eq('id', ex.id);
      return ex;
    }
  } catch (_) {}
  ...
  const payload = {
    ...
    repetition_level: quizData.repetition_level !== undefined ? Number(quizData.repetition_level) : 0,
    next_review_at: quizData.next_review_at || new Date(Date.now() + 24 * 3600 * 1000).toISOString()
  };
...
export function shuffleQuizOptions(options, correctIndex = 0) {
  if (!Array.isArray(options) || options.length <= 1) {
    return { options: options || [], correctIndex: 0 };
  }
  const safeCorrectIndex = (Number(correctIndex) >= 0 && Number(correctIndex) < options.length) ? Number(correctIndex) : 0;
...
/**
 * 🌟 Formats clean, un-truncated, beautiful text for Telegram's native quiz explanation popup (0-200 chars).
 * Strips raw HTML tags to prevent broken RTL tag flipping (e.g. </b>:ترجمة...<b>1.)
 * Highlights the concise correct answer and directs Dr. Abdallah to the full 5-point anatomical explanation below.
 */
export function formatNativePollExplanation(explanation, correctOptionText) {
  const optClean = (correctOptionText || '').replace(/<[^>]*>/g, '').trim();
  const shortOpt = optClean.length > 70 ? optClean.substring(0, 67) + '...' : optClean;

  // If explanation is plain text, short (<= 160 chars), and has no HTML tags or multi-paragraph formatting
  if (explanation && !/<[^>]*>/.test(explanation) && explanation.length <= 160 && !explanation.includes('\n\n')) {
    return `💡 ${explanation.trim()}`.substring(0, 195);
  }

  if (shortOpt) {
    return `💡 الإجابة: ${shortOpt}\n👇 الشرح السريري الخماسي الكامل والتحشيشة في الرسالة بالأسفل`.substring(0, 195);
  }

  return '💡 تم تسجيل إجابتك.\n👇 الشرح السريري الخماسي الكامل والتحشيشة في الرسالة بالأسفل'.substring(0, 195);
}

/**
 * 🌟 Extracts Arabic translation of clinical scenario from the 5-point explanation if present.
 */
export function extractArabicQuestionTranslation(explanation) {
...
export async function processStudentPollAnswer(pollId, selectedOptionIndex, telegramId) {
  const numId = Number(telegramId);
  try {
    // Search by embedded poll_id in doctor_pearl (quoted or unquoted)
    ...
    // Secondary fallback: search by telegram_poll_id column if present
    if (!quizzes || quizzes.length === 0) {
      const fb2 = await supabase
        .from('medical_spaced_quizzes')
        .select('*')
        .eq('telegram_poll_id', String(pollId))
        .limit(1);
      if (fb2.data && fb2.data.length > 0) {
        quizzes = fb2.data;
      }
    }
    ...
    if (!quiz) {
      console.warn(`[processStudentPollAnswer] Poll ID ${pollId} not found in database.`);
      return null;
    }
    ...
    if (quiz.doctor_pearl && quiz.doctor_pearl.includes('<<<QUIZ_META_START>>>')) {
      try {
        ...
        if (parsedMeta.explanation && parsedMeta.explanation.length > (explanation || '').length) {
          explanation = parsedMeta.explanation;
        }
      } catch (e) {}
    }

    // Ensure we always have the comprehensive explanation
    const fullExplanation = (explanation && explanation.length >= (quiz.answer_and_explanation || '').length)
      ? explanation
      : (quiz.answer_and_explanation || explanation);

    const isCorrect = Number(selectedOptionIndex) === Number(correctIdx);
    const isNonMed = ['SHARIA', 'STATESMAN', 'SAHABA_SPOTLIGHT', 'QURAN_ASBAB', 'BUKHARI', 'DISCIPLINE', 'PROPHETIC_LEADERSHIP'].includes((quiz.course_code || '').toUpperCase());

    let isMastered = false;
    let nextLevel = 0;
    let nextHours = 24;
    if (isCorrect) {
      nextLevel = Math.min(6, (quiz.repetition_level || 0) + 1);
      if (nextLevel >= 5 || (quiz.repetition_level || 0) >= 5 || isNonMed) {
        isMastered = true;
        nextHours = 8760; // 1 year (mastered permanently)
      } else {
        // Medical intervals: 1d (24h), 3d (72h), 7d (168h), 14d (336h), 30d (720h)
        const medIntervals = { 1: 24, 2: 72, 3: 168, 4: 336, 5: 720, 6: 1440 };
        nextHours = medIntervals[nextLevel] || 24;
      }
    } else {
      nextLevel = 0;
      nextHours = isNonMed ? 72 : 24; // 3 days for non-med, 24h for med review
    }
    const nextDate = isMastered
      ? new Date(Date.now() + 365 * 86400000).toISOString()
      : new Date(Date.now() + nextHours * 3600 * 1000).toISOString();

    // Update metadata inside doctor_pearl with student's last answer state
    let updatedDoctorPearl = quiz.doctor_pearl;
    if (quiz.doctor_pearl && quiz.doctor_pearl.includes('<<<QUIZ_META_START>>>')) {
      try {
        const parts = quiz.doctor_pearl.split('<<<QUIZ_META_START>>>');
        const endParts = parts[1].split('<<<QUIZ_META_END>>>');
        const meta = { ...parsedMeta };
        meta.last_answer_correct = isCorrect;
        meta.last_answered_at = new Date().toISOString();
        meta.selected_option_index = Number(selectedOptionIndex);
        meta.attempts = (meta.attempts || 0) + 1;
        if (isCorrect) meta.correct_count = (meta.correct_count || 0) + 1;
        else meta.wrong_count = (meta.wrong_count || 0) + 1;
        updatedDoctorPearl = `${parts[0]}<<<QUIZ_META_START>>>${JSON.stringify(meta)}<<<QUIZ_META_END>>>${endParts[1] || ''}`;
      } catch (e) {}
    }
    ...
    await supabase.from('medical_spaced_quizzes').update({
      ...
      doctor_pearl: updatedDoctorPearl,
      last_reviewed_at: new Date().toISOString()
    }).eq('id', quiz.id);
    ...
    return {
      ...
      quiz: {
        ...
        message_id: parsedMeta.message_id || null,
        chat_id: parsedMeta.chat_id || numId
      },
      nextLevel,
      nextDate
    };
...
export async function scheduleAdminSpacedQuiz(telegramId, { course_code = 'SHARIA', topic, question, options, correct_option_index = 0, explanation = '', delayDays = 7 }) {
  const numId = Number(telegramId);
  if (!question || !options || options.length < 2) return { success: false, skipped: true };

  // 🛡️ Minimum delay of 7 days (168 hours) to ensure true long-term memory retrieval without immediate spam
  const delayHours = Math.max(7 * 24, (delayDays || 7) * 24);
  ...
  try {
    const { data: existing } = await supabase
      .from('medical_spaced_quizzes')
      .select('id, repetition_level, is_mastered')
      .ilike('topic', `[UID:${numId}]%`)
      .eq('question', question)
      .limit(1);

    if (existing && existing.length > 0) {
      if (existing[0].is_mastered) return { success: true, alreadyMastered: true };
    ...
    const payload = {
      ...
      question,
      answer_and_explanation: explanation || 'شرح وتثبيت المفهوم',
...
// ==============================================================================
// 💎 31 & 32. منظومة التحشيشات الطبية وسجل أخطاء الأسئلة (Medical Mnemonics & Error Vault)
// ==============================================================================

import { MEDICAL_CURRICULUM } from './medical_curriculum.js';
import { MEDICAL_ESSENTIALS } from './medical_essentials.js';

export function normalizeMnemonicKey(text) {
  ...
  return text
    ...
    .replace(/ى/g, 'ي')
    .replace(/بـ\s*/g, 'ب')
    .replace(/:\s*(?:يعني|وده|علشان|عشان|أي)[\s\S]*/, '') // strip trailing explanation starting with colon
...
export async function saveMedicalMnemonic({ telegram_id, course_code = 'CAD402', topic, mnemonic_text, explanation_pearl, source = 'voice_note' }) {
  const numId = Number(telegram_id) || ADMIN_CHAT_ID;
  const cleanCourse = (course_code || 'CAD402').toUpperCase().trim();
  const cleanTopic = topic || 'تحشيشة سريرية';
  let cleanMnemonic = (mnemonic_text || '').trim();
  let cleanPearl = (explanation_pearl || '').trim();

  // Clean trailing explanation if appended after colon in mnemonic_text
  const colonMatch = cleanMnemonic.match(/^(.*?)(?::\s*(?:يعني|وده|علشان|عشان|أي)\s*([\s\S]*))$/);
  if (colonMatch) {
    cleanMnemonic = colonMatch[1].trim();
    if (!cleanPearl && colonMatch[2]) {
      cleanPearl = colonMatch[2].trim();
    }
  }

  if (!cleanMnemonic) return { success: false, error: 'نص التحشيشة فارغ' };

  const normKey = normalizeMnemonicKey(cleanMnemonic);
  const coreKey = normKey.slice(0, 25);

  // 0. Smart Deduplication: Check if mnemonic already exists in thoughts_and_wisdom fallback
  try {
    const { data: existingRows } = await supabase.from('thoughts_and_wisdom')
      .select('id, content')
      .eq('category', 'تحشيشة ونيمونيك طبي');
      ...
      for (const row of existingRows) {
        try {
          ...
          if (parsed && parsed.mnemonic_text) {
            const exNorm = normalizeMnemonicKey(parsed.mnemonic_text);
            const exCore = exNorm.slice(0, 25);
            const isMatch = (coreKey && exCore && (coreKey === exCore || normKey.includes(exCore) || exNorm.includes(coreKey))) ||
              (cleanTopic && parsed.topic && cleanTopic.toLowerCase() === parsed.topic.toLowerCase() && coreKey.slice(0, 15) === exCore.slice(0, 15));

            if (isMatch) {
              // Update with richer explanation if needed
              if (cleanPearl && (!parsed.explanation_pearl || cleanPearl.length > parsed.explanation_pearl.length)) {
                parsed.explanation_pearl = cleanPearl;
                parsed.topic = cleanTopic || parsed.topic;
                await supabase.from('thoughts_and_wisdom').update({
                  content: JSON.stringify(parsed)
                }).eq('id', row.id);
              }
              return { success: true, data: { ...parsed, id: row.id }, id: row.id, storage: 'deduplicated_existing' };
            }
          }
        } catch (_) {}
      }
    ...
    console.warn('Mnemonic fallback dedup check warn:', dedupErr.message);
  }

  try {
    const { data, error } = await supabase.from('medical_mnemonics').insert({
      telegram_id: numId,
      course_code: cleanCourse,
      topic: cleanTopic,
      mnemonic_text: cleanMnemonic,
      explanation_pearl: cleanPearl || null,
      source: source || 'voice_note'
    }).select().single();

    if (!error && data) {
      return { success: true, data, storage: 'medical_mnemonics' };
    }

    // Fallback if table does not exist in schema cache
    if (error && (error.code === 'PGRST205' || error.message?.includes('not find the table') || error.code === '42P01')) {
      const fallbackPayload = {
        ...
        topic: cleanTopic,
        mnemonic_text: cleanMnemonic,
        explanation_pearl: cleanPearl,
        source,
        created_at: new Date().toISOString()
      };

      try {
        const { data: insData } = await supabase.from('thoughts_and_wisdom').insert({
          content: JSON.stringify(fallbackPayload),
          category: 'تحشيشة ونيمونيك طبي',
          tags: ['mnemonic', cleanCourse, `usr:${numId}`]
        }).select().single();
        if (insData) {
          fallbackPayload.id = insData.id;
        }
      } catch (fbErr) {
        console.warn('Fallback mnemonic insert warning:', fbErr.message);
      }

      return { success: true, data: fallbackPayload, id: fallbackPayload.id, storage: 'thoughts_and_wisdom_fallback' };
    }

    return { success: false, error: error?.message };
  } catch (err) {
    console.warn('saveMedicalMnemonic error:', err.message);
    return { success: false, error: err.message };
  }
}

export async function getMedicalMnemonics(telegramId, courseCode = null, limit = 60) {
  const numId = Number(telegramId) || ADMIN_CHAT_ID;
  const filterCourse = courseCode ? courseCode.toUpperCase().trim() : null;
  const results = [];
  const seenKeys = new Set();

  // 1. Try fetching from medical_mnemonics table
  try {
    let query = supabase.from('medical_mnemonics').select('*');
    if (numId !== ADMIN_CHAT_ID) {
      query = query.or(`telegram_id.eq.${numId},telegram_id.eq.${ADMIN_CHAT_ID}`);
    }
    ...
    if (!error && Array.isArray(data)) {
      data.forEach(item => {
        const key = normalizeMnemonicKey(item.mnemonic_text).slice(0, 25) || (item.topic || '').toLowerCase().trim();
        if (key && !seenKeys.has(key)) {
          ...
          results.push({
            ...
            explanation_pearl: item.explanation_pearl,
            source: item.source,
    ...
    console.warn('Error reading medical_mnemonics:', err.message);
  }

  // 2. Fetch from thoughts_and_wisdom fallback
  try {
    let fbQuery = supabase.from('thoughts_and_wisdom')
      .select('*')
      .eq('category', 'تحشيشة ونيمونيك طبي')
      .order('created_at', { ascending: false })
      .limit(limit);
    ...
    if (Array.isArray(fbData)) {
      fbData.forEach(row => {
        try {
          const parsed = JSON.parse(row.content);
          if (parsed && parsed.mnemonic_text) {
            const course = parsed.course_code || 'CAD402';
            if (!filterCourse || filterCourse === 'ALL' || course === filterCourse) {
              const key = normalizeMnemonicKey(parsed.mnemonic_text).slice(0, 25) || (parsed.topic || '').toLowerCase().trim();
              if (key && !seenKeys.has(key)) {
                seenKeys.add(key);
                results.push({
                  id: row.id,
                  ...
                  explanation_pearl: parsed.explanation_pearl || null,
    ...
    console.warn('Error reading fallback mnemonics:', err.message);
  }

  // 3. Integrate built-in curriculum mnemonics (from MEDICAL_CURRICULUM)
  if (Array.isArray(MEDICAL_CURRICULUM)) {
    MEDICAL_CURRICULUM.forEach(item => {
      const q = item.quiz;
      const mText = q?.mnemonic_scheme || item.mnemonic_scheme;
      const course = (item.course_code || 'CAD402').toUpperCase();
      if (mText && (!filterCourse || filterCourse === 'ALL' || course === filterCourse || (filterCourse === 'CAD402' && course === 'CAD'))) {
        const key = normalizeMnemonicKey(mText).slice(0, 25) || (item.title || item.topic || '').toLowerCase().trim();
        if (key && !seenKeys.has(key)) {
          seenKeys.add(key);
          results.push({
            id: `curriculum_${item.id}`,
            course_code: course === 'CAD' ? 'CAD402' : (course === 'PED' ? 'PED401' : course),
            topic: item.title || item.topic,
            mnemonic_text: mText,
            explanation_pearl: q?.options_analysis?.correct || (typeof item.summary_card === 'string' ? item.summary_card.slice(0, 180) + '...' : ''),
            source: 'curriculum',
  ...
  // 4. Integrate emergency essentials mnemonics (from MEDICAL_ESSENTIALS)
  if (Array.isArray(MEDICAL_ESSENTIALS)) {
    MEDICAL_ESSENTIALS.forEach(item => {
      if (item.mnemonic_scheme) {
        const course = 'SKL 7';
        if (!filterCourse || filterCourse === 'ALL' || filterCourse === 'SKL 7') {
          const key = normalizeMnemonicKey(item.mnemonic_scheme).slice(0, 25) || (item.title || '').toLowerCase().trim();
          if (key && !seenKeys.has(key)) {
            seenKeys.add(key);
            results.push({
              id: `essentials_${item.id}`,
              course_code: 'SKL 7',
              topic: item.title,
              mnemonic_text: item.mnemonic_scheme,
              explanation_pearl: item.clinical_pearl_egyptian || item.emergency_scenario,
              source: 'essentials',
              created_at: '2026-09-01T00:00:00Z',
              type: 'curriculum'
            });
...
export async function saveMedicalErrorVault({
  telegram_id,
  course_code = 'CAD402',
  topic,
  question_en,
  ...
  const numId = Number(telegram_id) || ADMIN_CHAT_ID;
  const cleanCourse = (course_code || 'CAD402').toUpperCase().trim();
  const cleanTopic = topic || 'خطأ في سؤال طبي سريري';

  // 0. Smart Deduplication: Check if question already exists in thoughts_and_wisdom
  const qStem = (question_en || question_ar || '').replace(/[^\p{L}\p{N}]/gu, '').toLowerCase().slice(0, 45);
  try {
    const { data: existingVaultRows } = await supabase.from('thoughts_and_wisdom')
      .select('id, content')
      .eq('category', 'سجل أخطاء الأسئلة الطبية');

    if (Array.isArray(existingVaultRows)) {
      for (const row of existingVaultRows) {
        try {
          const parsed = JSON.parse(row.content);
          const exStem = (parsed.question_en || parsed.question_ar || '').replace(/[^\p{L}\p{N}]/gu, '').toLowerCase().slice(0, 45);
          if (qStem && exStem && (qStem === exStem || qStem.slice(0, 30) === exStem.slice(0, 30))) {
            return { success: true, data: { ...parsed, id: row.id }, id: row.id, storage: 'deduplicated_existing' };
          }
        } catch (_) {}
      }
    ...
    console.warn('Vault dedup check warn:', dedupVErr.message);
  }

  // 1. Save Mnemonic if exists
  let savedMnemonicId = null;
  if (mnemonic_text && mnemonic_text.trim()) {
    const pearl = golden_tip || correct_mechanism || '';
    const mRes = await saveMedicalMnemonic({
      telegram_id: numId,
      course_code: cleanCourse,
      topic: cleanTopic,
    ...
    if (mRes.success && mRes.data?.id) savedMnemonicId = mRes.data.id;
  }

  // 2. Schedule in medical_spaced_quizzes for active recall
  try {
    const options = [correct_answer];
    if (Array.isArray(distractors)) {
      distractors.forEach(d => {
        const optText = typeof d === 'string' ? d : (d.option || d.text || '');
        if (optText && !options.includes(optText)) options.push(optText);
      });
    }

    if (options.length >= 2) {
      await scheduleAdminSpacedQuiz(numId, {
        course_code: cleanCourse,
        topic: cleanTopic,
        question: question_en || question_ar,
        options,
        correct_option_index: 0,
        explanation: `${correct_mechanism || ''}\n\n💡 التحشيشة: ${mnemonic_text || ''}\n🌟 نصيحة: ${golden_tip || ''}`,
        delayDays: 1
      });
    }
  } catch (err) {
    console.warn('schedule quiz from error vault error:', err.message);
  }

  // 3. Save to medical_error_vault table
  try {
    const payload = {
      telegram_id: numId,
      course_code: cleanCourse,
      ...
      difficult_terms: Array.isArray(difficult_terms) ? difficult_terms : [],
      correct_answer: correct_answer || '',
      correct_mechanism: correct_mechanism || '',
      distractors: Array.isArray(distractors) ? distractors : [],
      curriculum_context: curriculum_context || null,
      golden_tip: golden_tip || null,
      mnemonic_id: savedMnemonicId,
    ...
    const { data, error } = await supabase.from('medical_error_vault').insert(payload).select().single();
    if (!error && data) {
      return { success: true, data, id: data.id, storage: 'medical_error_vault' };
    }

    // Fallback if table does not exist in schema cache
    if (error && (error.code === 'PGRST205' || error.message?.includes('not find the table') || error.code === '42P01')) {
      const fbId = `vault_fb_${Date.now()}`;
      try {
        await supabase.from('thoughts_and_wisdom').insert({
          content: JSON.stringify({ ...payload, id: fbId }),
          category: 'سجل أخطاء الأسئلة الطبية',
          tags: ['error_vault', cleanCourse, `usr:${numId}`]
        });
      } catch (fbErr) {
        console.warn('Fallback error vault insert warning:', fbErr.message);
      }

      return { success: true, data: { ...payload, id: fbId }, id: fbId, storage: 'thoughts_and_wisdom_fallback' };
    }

    return { success: false, error: error?.message };
  } catch (err) {
    console.warn('saveMedicalErrorVault error:', err.message);
    return { success: false, error: err.message };
  }
}

export async function getMedicalErrorVault(telegramId, courseCode = null, limit = 50) {
  const numId = Number(telegramId) || ADMIN_CHAT_ID;
  const filterCourse = courseCode ? courseCode.toUpperCase().trim() : null;
  const results = [];
  ...
  // 1. Try fetching from medical_error_vault table
  try {
    let query = supabase.from('medical_error_vault').select('*');
    if (numId !== ADMIN_CHAT_ID) {
      query = query.or(`telegram_id.eq.${numId},telegram_id.eq.${ADMIN_CHAT_ID}`);
    }
    if (filterCourse && filterCourse !== 'ALL') {
    ...
    if (!error && Array.isArray(data)) {
      data.forEach(item => {
        const stem = (item.question_en || item.question_ar || item.topic || '').replace(/[^\p{L}\p{N}]/gu, '').toLowerCase().slice(0, 45);
        if (!seenIds.has(item.id) && (!stem || !seenStems.has(stem))) {
          seenIds.add(item.id);
    ...
    console.warn('Error reading medical_error_vault:', err.message);
  }
  ...
  try {
    ...
    if (Array.isArray(fbData)) {
      fbData.forEach(row => {
        try {
          const parsed = JSON.parse(row.content);
          if (parsed && (parsed.question_en || parsed.question_ar)) {
            const course = (parsed.course_code || 'CAD402').toUpperCase();
            if (!filterCourse || filterCourse === 'ALL' || course === filterCourse) {
              const id = parsed.id || row.id;
              const stem = (parsed.question_en || parsed.question_ar || parsed.topic || '').replace(/[^\p{L}\p{N}]/gu, '').toLowerCase().slice(0, 45);
              if (!seenIds.has(id) && (!stem || !seenStems.has(stem))) {
                seenIds.add(id);
                if (stem) seenStems.add(stem);
                results.push({
                  id,
                  course_code: course,
                  topic: parsed.topic || 'خطأ في سؤال سريري',
    ...
    console.warn('Error reading fallback error vault:', err.message);
  }
