import { supabase, getAllRegisteredUsers, getUserProfile, getGenderTerms, DEFAULT_USER_PREFERENCES, ADMIN_CHAT_ID, shuffleQuizOptions, formatNativePollExplanation } from './supabase.js';
      ...
      if (validQuizzes && validQuizzes.length > 0) {
        ...
        if (!alreadySent) {
          ...
          if (options && options.length >= 2) {
            ...
            let pollPrompt = '';
            // If the clinical question is long (> 200 chars), put the FULL scenario in introMsg so it never gets cut off!
            if (quiz.question.length > 200) {
              let introMsg = `🩺 <b>كويز طبي سريري [${quiz.course_code || 'MED'}] 🧠✨</b>\n`;
              introMsg += `━━━━━━━━━━━━━━━━━━━━━\n`;
              introMsg += `📌 <b>الموضوع:</b> <b>${cleanTopic}</b>\n\n`;
              introMsg += `📋 <b>الحالة والسيناريو السريري (Clinical Scenario):</b>\n`;
              introMsg += `<i>${quiz.question}</i>\n\n`;
              introMsg += `👇 <i>اختر الإجابة أو التشخيص الصحيح من الاستطلاع أدناه (+30 Doctor XP):</i>`;
              await bot.telegram.sendMessage(chatId, introMsg, { parse_mode: 'HTML' }).catch(() => {});

              // Extract the final question clause or build a concise prompt
          ...
          if (dueNonMedQuizzes && dueNonMedQuizzes.length > 0) {
            ...
            if (options && options.length >= 2) {
              ...
              if (quiz.question.length > 200) {
                ...
                introMsg += `📋 <b>نص السؤال كاملاً:</b>\n<i>${quiz.question}</i>\n\n`;
                introMsg += `👇 <i>اختر الإجابة الصحيحة من الاستطلاع أدناه:</i>`;
                await bot.telegram.sendMessage(chatId, introMsg, { parse_mode: 'HTML' }).catch(() => {});

                const sentences = quiz.question.split(/[.?؟]/).map(s => s.trim()).filter(Boolean);
                const lastSentence = sentences.length > 1 ? sentences[sentences.length - 1] : '';
                if (lastSentence.length >= 15 && lastSentence.length <= 250) {
                  pollPrompt = `${lastSentence}؟`;
                } else {
                  pollPrompt = `${cleanTopic}: ما هي الإجابة الصحيحة؟`;
                }
              } else {
