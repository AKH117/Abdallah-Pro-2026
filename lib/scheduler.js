...
async function runUserCycle(bot, user) {
  ...
  try {
    ...
    const isCapsuleWindow = (curMinutes >= 560 && curMinutes <= 610) || // 09:30 الشريعة
                            (curMinutes >= 665 && curMinutes <= 715) || // 11:15 الانضباط
                            (curMinutes >= 800 && curMinutes <= 850) || // 01:30 آية قرآنية
                            (curMinutes >= 935 && curMinutes <= 985) || // 03:45 رجل الدولة
    ...
    if (curMinutes >= 570 && curMinutes <= 600) {
      ...
      if (await claimNotification(shariaKey, 86400000)) {
        const capsule = await getNextShariaCapsule(adminChatId);
    ...
    if (curMinutes >= 945 && curMinutes <= 975) {
      ...
      if (await claimNotification(statesmanKey, 86400000)) {
        const mistake = await getNextPastRulerMistake(adminChatId);
