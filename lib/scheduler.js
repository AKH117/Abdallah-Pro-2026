...
async function runUserCycle(bot, user) {
  ...
  if (prefs.english !== false && prefs.english_auto_review !== false) {
    try {
      // Send at most ONE English flashcard review per day to avoid spamming the student
      const dailyUserKey = `eng_spaced_user_${chatId}_${dateStr}`;
      const claimed = await claimNotification(dailyUserKey, 20 * 3600 * 1000);
      ...
      if (claimed) {
        ...
        if (dueCards && dueCards.length > 0) {
          ...
          // Push next review forward by 24 hours so unreviewed cards never repeat within the same day
          await supabase
            .from('english_spaced_flashcards')
            .update({
