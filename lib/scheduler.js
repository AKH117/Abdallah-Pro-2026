...
import { supabase, getAllRegisteredUsers, getUserProfile, getGenderTerms, DEFAULT_USER_PREFERENCES, ADMIN_CHAT_ID, shuffleQuizOptions, scheduleAdminSpacedQuiz } from './supabase.js';
...
async function runUserCycle(bot, user) {
  ...
  // ============================================================================
  // 9. 👑 التوزيع الذكي التلقائي لمنظومة الكبسولات الإضافية (تم إيقافها نهائياً)
  // ============================================================================
  // 🛑 تم إيقاف جميع الكبسولات الإضافية والكويزات المتباعدة غير الطبية نهائياً بناءً على رغبة د. عبدالله
  // المنظومة مقتصرة حصرياً على: الطب، الإنجليزية، والصلوات ومواقيت الإقامة اليومية.
  return;
  ...
  try {
    ...
    // 🛡️ حماية صارمة: ممنوع إرسال أي كويز في نوافذ إرسال الكبسولات الصباحية والمسائية لمنع تداخل الرسائل إطلاقاً!
    const isCapsuleWindow = (curMinutes >= 665 && curMinutes <= 715) || // 11:15 الانضباط
                            (curMinutes >= 800 && curMinutes <= 850) || // 01:30 آية قرآنية
                            (curMinutes >= 1025 && curMinutes <= 1075) || // 05:15 الصحابي
                            (curMinutes >= 1130 && curMinutes <= 1180) || // 07:00 صحيح البخاري
                            (curMinutes >= 1205 && curMinutes <= 1255) || // 08:15 الورد القرآني
                            (curMinutes >= 1265 && curMinutes <= 1315);   // 09:15 وقود النقاء

    if (!isCapsuleWindow) {
      try {
        const nowIso = new Date().toISOString();
        const allowedAdminCodes = ['SAHABA_SPOTLIGHT', 'QURAN_ASBAB', 'BUKHARI', 'DISCIPLINE', 'PROPHETIC_LEADERSHIP'];
        const { data: dueQuizzes } = await supabase
          .from('medical_spaced_quizzes')
          .select('*')
          .ilike('topic', `[UID:${adminChatId}]%`)
          .in('course_code', allowedAdminCodes)
          .lte('next_review_at', nowIso)
          .eq('is_mastered', false)
          .order('next_review_at', { ascending: true })
          .limit(5);

        // 🛡️ فلترة صارمة: استبعاد أسئلة الفقه والشريعة وإصلاح رجل الدولة وأخطاء الذين مضوا نهائياً بناءً على طلب د. عبدالله
        const validQuizzes = (dueQuizzes || []).filter(q => 
          q.course_code !== 'SHARIA' && 
          q.course_code !== 'STATESMAN' &&
          !/فقه|شريع|دولة|مضوا/.test(q.topic || '') &&
          !/فقه|شريع|دولة|مضوا/.test(q.question || '')
        );

        if (validQuizzes && validQuizzes.length > 0) {
          const qRow = validQuizzes[0];
          const quizKey = `admin_due_quiz_sent_${qRow.id}_${dateStr}`;
          if (await claimNotification(quizKey, 86400000)) {
            let meta = {};
            ...
            if (meta.options && meta.options.length >= 2) {
              ...
              // 🎲 Shuffle options before sending so correct answer is randomly distributed (never fixed to option 1)
              const shuffled = shuffleQuizOptions(meta.options, Number(meta.correct_index || 0));
              const sendOptions = shuffled.options.map(o => String(o).substring(0, 100));
              const sendCorrectId = Math.max(0, Math.min(sendOptions.length - 1, shuffled.correctIndex));
              ...
              const pollMsg = await bot.telegram.sendQuiz(
                ...
                sendOptions,
                {
                  correct_option_id: sendCorrectId,
                  is_anonymous: false,
              ...
              if (pollMsg && pollMsg.poll) {
                ...
                meta.options = sendOptions;
                meta.correct_index = sendCorrectId;
                // Hold next review forward 24h to avoid resending today if student hasn't answered; DO NOT increment level until answered!
                const holdReviewDate = new Date(Date.now() + 24 * 3600 * 1000).toISOString();

                await supabase.from('medical_spaced_quizzes').update({
                  next_review_at: holdReviewDate,
    ...
    if (curMinutes >= 675 && curMinutes <= 705) {
      ...
      if (await claimNotification(discKey, 86400000)) {
        const disc = await getNextScientificDiscipline(adminChatId);
        ...
        // ⏳ Schedule quiz 2 hours after capsule delivery (+2h delay)
    ...
    if (curMinutes >= 810 && curMinutes <= 840) {
      ...
      if (await claimNotification(ayahKey, 86400000)) {
        const ayah = await getNextAyahWithAsbab(adminChatId);
    ...
    if (curMinutes >= 1035 && curMinutes <= 1065) {
      ...
      if (await claimNotification(sahabiKey, 86400000)) {
        const sahabi = await getNextSahabiSpotlight(adminChatId);
    ...
    if (curMinutes >= 1140 && curMinutes <= 1170) {
      ...
      if (await claimNotification(bukhariKey, 86400000)) {
        const bukhari = await getNextBukhariHadith(adminChatId);
    ...
    // 🛑 A. كبسولة العلم الشرعي والفقه (09:30 صباحاً) -> [تم إيقاف الإشعارات والأسئلة نهائياً بناءً على رغبة د. عبدالله]
    // 🛑 B. فقرة أخطاء الذين مضوا وإصلاح رجل الدولة (03:45 عصراً) -> [تم إيقاف الإشعارات والأسئلة نهائياً بناءً على رغبة د. عبدالله]
    ...
    if (curMinutes >= 1215 && curMinutes <= 1245) {
      ...
      if (await claimNotification(healKey, 86400000)) {
    ...
    if (curMinutes >= 1275 && curMinutes <= 1305) {
      ...
      if (await claimNotification(fuelKey, 86400000)) {
        const fuel = await getNextPuritySpiritualFuel(adminChatId, 'د. عبدالله');
